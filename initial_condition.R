...
###add inclination and Omega
        if(out$astro.par>0 | length(out$rel)>0){
...
#####epoch absolute astrometry data
    if(length(out$data.epoch)>0 & FALSE){
...
#if((any(names(out)=='rel') | any(out$comp.epoch==2)) & !any(par.fix=='Mstar')){
...
if(dtype=='raw' & out$Nrv>0){
    for(i in out$ins.rv){
###offsets of RV sets
        par.ini <- c(par.ini,0)
        par.min <- c(par.min,-1e6)
        par.max <- c(par.max,1e6)
        nams <- c(nams,paste0('rv0_',i))
    }
    if(length(out$data.binary)>0){
        if(length(out$data.binary$RV)>0){
            for(i in unique(out$data.binary$ins)){
                par.ini <- c(par.ini,0)
                par.min <- c(par.min,-1e6)
                par.max <- c(par.max,1e6)
                nams <- c(nams,paste0('rv0b_',i))
            }
        }
    }
}
...
###offsets of RV sets
        ...
        nams <- c(nams,paste0('bb_',ins.binary))
    }
###white jitter
    ...
    nams <- c(nams,paste0('Jb_',out$ins.binary))
}
