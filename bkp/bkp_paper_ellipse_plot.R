...
rel.sim <- astrometry.rel(par.opt,tt=tsim)$rel
ins <- out$ins.epoch
...
#    plot(dra,ddec,xlab=expression(Delta*alpha*'* [mas]'),ylab=expression(Delta*delta*' [mas]'),xlim=rev(range(dra,dra.sim[i2])),ylim=range(ddec,ddec.sim[i2]),pch=ell.pch,col='grey',main='Hipparcos IAD')
    plot(dra,ddec,xlab=expression(Delta*alpha*'* [mas]'),ylab=expression(Delta*delta*' [mas]'),xlim=xlim,ylim=ylim,pch=ell.pch,col='grey',main='Hipparcos IAD',xaxt='n',yaxt='n')
    par(xpd=FALSE)
...
#            segments(dra[jj[j1-1]],ddec[jj[j1-1]],dra[jj[j1]],ddec[jj[j1]],col='grey',lwd=0.5,lty=2)
    ...
    if(FALSE){
        cc <- paletteer_c(palette = "viridis::inferno", n = length(tsim))
        kk <- as.factor( as.numeric( cut((tsim-Tascend)%%Popt[j5], length(tsim))))
        i3 <- sapply(tbin,function(t) which.min(abs(tsim-t)))
    }else{
...
plot(reflex.sim[,'dra'],reflex.sim[,'ddec'],xlab=expression(Delta*alpha*'* [mas]'),ylab=expression(Delta*delta*' [mas]'),type='l',col=fit.opt,xlim=xlim,ylim=ylim,main='Gaia GOST',lwd=2,xaxt='n',yaxt='n')
par(xpd=FALSE)
magaxis(1:2,cex=size)
magaxis(3:4,cex=size,labels=FALSE)
ind <- which.min(abs((tsim-Tascend)%%Popt[j5]))
...
#legend('top',legend=phases,pch=20,col=cl[as.factor(phases)],horiz=TRUE,bty='n',inset=c(0,-0.22),xpd=NA,title='Phase')
ref.epochs <- c()
...
astro <- calc.astro(K=Ks,P=ps,e=es,inc=incs,omega=omegas,Omega=Omegas,E=Es,plx=out$plx,Mstar=mstars,eta=0)
...
plot(dra.sim,ddec.sim,xlab=expression(Delta*alpha*'* [mas]'),ylab=expression(Delta*delta*' [mas]'),type='l',col=fit.opt,main='Planet position on 2024-01-01',lwd=2,xlim=xlim,ylim=ylim,xaxt='n',yaxt='n')
par(xpd=FALSE)
magaxis(1:2,cex=size)
magaxis(3:4,cex=size,labels=FALSE)
ind <- which.min(abs((tsim-Tascend)%%Popt[j5]))
