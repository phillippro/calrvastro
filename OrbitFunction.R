...
####galactic velocity to equatorial velocity in the helio-centric frame
g2e.vel <- function(vx,vy,vz){
    v <- sqrt(vx^2+vy^2+vz^2)
    bl <- xyz2bl.vec(vx,vy,vz)
    ad <- gal2equ(bl[,1],bl[,2])
    vxe <- v*cos(ad[,2])*cos(ad[,1])
    vye <- v*cos(ad[,2])*sin(ad[,1])
    vze <- v*sin(ad[,2])
