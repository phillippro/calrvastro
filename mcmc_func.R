...
auyr2kms <- 4.74047
SRS <- 1.97412574336e-8#au; solar schwarzschild radius
...
assign.names <- function(par.vector,Np,out1=out,p=0,q=0,n=0,bases=rep('natural',10)){
    nams <- c()
    #names for keplerian part
    if(Np>0 | out1$Nm>0){
        for(j in 1:(Np+out1$Nm)){
            if(bases[j]=='natural'){
                if(prior.type=='e0'){
    ...
    if(length(p)>0 & length(out1$Nrv)>0){
...
#    if(any(names(out)=='astrometry') & !grepl('-',astro.type)){
    if(length(out1$astrometry)>0){
        nams <- c(nams,'J_gaia')
        if(length(out1$ins.epoch)>0){
            nams <- c(nams,paste0('J_',out1$ins.epoch))
        }else if(!is.null(out1$ihip) & !any(grepl('hip',out1$ins.epoch))){
            nams <- c(nams,'J_hip')
        }
        if(length(out1$gost)>0 | length(out1$data.epoch)>0){
...
#    if(any(names(out)=='rel'))  nams <- c(nams,c('J_rel'))
    ...
    if(out1$Nrvc>0){
        if(rvc.type=='relative'){
            nams <- c(nams,paste0('bc',out1$Irvc))
        }else{
            nams <- c(nams,'rvb')
        }
    }
...
####reflex motion
   if(length(out1$data.epoch)>0){
       nams <- c(nams,paste0('J_',out1$ins.epoch))
       ind <- which(!grepl('hip',out1$ins.epoch))
       if(length(ind)>0){
#           nams <- c(nams,paste0('dra_',out1$ins.epoch[ind]))
#           nams <- c(nams,paste0('ddec_',out1$ins.epoch[ind]))
       }
   }

#   if(length(out1$data.ref)>0){
#       nams <- c(nams,paste0('J_',out1$ins.ref))
#   }
    if(photovary){
       nams <- c(nams,'eta')
    }
    if(length(out1$data.binary)>0){
        if(offset) nams <- c(nams,paste0('b_',out1$ins.binary))
        nams <- c(nams,paste0('s_',out1$ins.binary))
    }
    if(FALSE){
#    if(TRUE){
            ...
            for(k in 1:length(ii)){
                ...
                if(grepl('[1-9]$',ii[k])){
                    if(length(ss)>1){
                        ...
                        ilow <- ''
                        iup <- ss
                        if(grepl('[1-9]',iup)) iup <- out1$ins.rv[as.numeric(iup)]
...
extract.par <- function(pars.kep,out1=out,bases=NULL){
...
###calculate POS
    ...
    Y <- sqrt(1-e^2)*sin(E)
    ##    T <- 2*atan(sqrt((1+e)/(1-e))*tan(E/2))
    ##    alpha <- (P/365.25)^{2/3}*mp/(out1$Mstar+mp)^{2/3}*plx#semi-major axis in mas
...
if(FALSE){
    ...
    if(state){
###Note: the basis vectors for this state vector should be pqu at the reference epoch
        BT <- cbind(raP/plx,decP/plx,beta0*(C*X+H*Y),pmraP/plx,pmdecP/plx,rv)#au,au/yr
        return(BT)
    }else{
        return(data.frame(ra=raP,dec=decP,plx=plxP,pmra=pmraP,pmdec=pmdecP,rv=rv*4.74047))#mas,mas/yr; rv: km/s
    }
}
...
##t is in unit of day
##obs: ra(deg), dec(deg),plx(mas),pmra(mas/yr),pmdec(mas/yr), rv(km/s)
...
###state -> obs: velocity to pm
        ...
        rv1 <- vequ[,1]*auyr2kms#au/yr->km/s
        out <- cbind(ra1,dec1,1/d1,pmra1,pmdec1,rv1,x1,y1,z1,vx.equ,vy.equ,vz.equ)#position unit: pc; velocity unit: au/yr
        colnames(out) <- c('ra','dec','parallax','pmra','pmdec','radial_velocity','x','y','z','vx','vy','vz')
    }else{
...
astrometry.rel <- function(pars.kep,tt=NULL,out1=out,bases=rep('natural',10),pp=NULL,eta=0,band='G'){
    if(is.null(pp))  pp <- extract.par(pars.kep,out1=out1,bases=bases)
...
####relative astrometry reference: https://arxiv.org/pdf/2109.10671.pdf
            if(!is.null(tt)){
                ms <- (pp$Mo[j]+2*pi*(tt-out1$tref)/pp$P[j])%%(2*pi)
            }else{
                ms <- (pp$Mo[j]+2*pi*(out1$tauT[out1$ind.rel]-out1$tref)/pp$P[j])%%(2*pi)
            }
            E <- kep.mt2(ms,pp$e[j])
            plx <- out1$plx
...
#            cat(parin,'\n')
            tmp <- calc.astro(pp,index=j,E,plx=as.numeric(plx),Mstar=mm,pars=pars.kep,eta=eta)
            ...
            }else if(length(out1$indrel[[n]])>0){
                for(i in inss){
                    raB[[n]][[i]] <- -tmp[out1$indrel[[n]][[i]],1]/xi
                    decB[[n]][[i]] <- -tmp[out1$indrel[[n]][[i]],2]/xi
                    pmraB[[n]][[i]] <- -tmp[out1$indrel[[n]][[i]],4]/xi
                    pmdecB[[n]][[i]] <- -tmp[out1$indrel[[n]][[i]],5]/xi
                    if(kk>1){
                        raB[[n]][[i]] <- raB[[n]][[i]] - rowSums(raR[out1$indrel[[n]][[i]],1:(kk-1),drop=FALSE])
                        decB[[n]][[i]] <- decB[[n]][[i]] - rowSums(decR[out1$indrel[[n]][[i]],1:(kk-1),drop=FALSE])
                        pmraB[[n]][[i]] <- pmraB[[n]][[i]]- rowSums(pmraR[out1$indrel[[n]][[i]],1:(kk-1),drop=FALSE])
                        pmdecB[[n]][[i]] <- pmdecB[[n]][[i]] - rowSums(pmdecR[out1$indrel[[n]][[i]],1:(kk-1),drop=FALSE])
                    }
                }
            }
        }
...
#    if(TRUE){
        ...
        cat('Omega=',pp$Omega,'\n')
        cat('omega=',pp$omega,'\n')
        dra.obs <- out$rel$p1$K17$dra
        ddec.obs <- out$rel$p1$K17$ddec
        if(is.null(tt)){
            plot(raB$p1$K17,decB$p1$K17,main='model(black)',xlim=rev(range(dra.obs,raB$p1$K17)),ylim=range(decB$p1$K17,ddec.obs))
        }else{
            plot(raB$p1,decB$p1,main='model(black)',xlim=rev(range(dra.obs,raB$p1)),ylim=range(decB$p1,ddec.obs))
        }
        points(dra.obs,ddec.obs,col='red')
        dev.off()
    }
        ...
        for(j in 1:Np.kep){
            if(pp$P[j]>Pmin){
                if(!is.null(tt)){
                    ...
                    ms <- (pp$Mo[j]+2*pi*(out1$data.ref[,1]-out1$tref)/pp$P[j])%%(2*pi)
                }
                E <- kep.mt2(ms,pp$e[j])
...
astrometry.epoch <- function(pars.kep,out1=out,tt=NULL,bases=rep('natural',10),pp=NULL,eta=0,barycenter=NULL,band='G',comp=1){
    if(is.null(pp))  pp <- extract.par(pars.kep,bases=bases)
#    ra.planet <- dec.planet <- pmra.planet <- pmdec.planet <- 0
    Np.kep <- length(grep('omega',names(pars.kep)))
...
###https://ui.adsabs.harvard.edu/abs/2016MNRAS.455.4136B/abstract
    ...
    tau <- t-roemerT#barycentric time to emission time; without relativitistic correction
    return(data.frame(tau=tau,roemer=roemerT))
}

###This is to account for the relativitiy-induced RV variation in the target system
RV.relativity <- function(pars.kep,out1=out,t=NULL,indP=NULL,indC=NULL,Mstar=NULL,Mps=NULL,as=NULL,bases=rep('natural',10),pp=NULL,gamma=1,sim.kep=FALSE,tol=1e-3){
###Note: Here the variables with T could represent both primary P or companion C.
####initial settings
    if(is.null(pp))  pp <- extract.par(pars.kep,out1=out1,bases=bases)
    binarity <- FALSE
    if(length(out1$data.binary)>0) binarity <- TRUE
    plx <- out1$plx
    if(any(names(pars.kep)=='dplx')) plx <- plx-pars.kep['dplx']
    rv <- 0
    Np.kep <- length(grep('omega',names(pars.kep)))
...
#####iterations to calulate BT
    ...
    tauT <- t
    for(k in 1:Ntry){
#t already correct for solar system roemer delay
        tauT <- t-roemerT#include both tauC and tauT and will be separated after the iteration
        zgs <- EP <- list()
        BT <- 0
        zgT <- roemerT <- 0
        for(j in 1:Np.kep){
###calculate BT
            if(pp$K[j]>0){
                m <- (pp$Mo[j]+2*pi*(tauT-out1$tref)/pp$P[j])%%(2*pi)
                EP[[j]] <- kep.mt2(m,pp$e[j])%%(2*pi)
                bt <- calc.astro(pp,j,EP[[j]],plx=plx,Mstar=Mstar,state=TRUE,eta=0,pars=pars.kep)
                BT <- BT+bt
                roemerT <- roemerT+rowSums(bt[,1:3]*out1$uOB)/Cauday#approximation: actually bt be projected onto uOT or uOC to get relevant delay
                eta <- Mps[j]/(Mstar+Mps[j])
                RCT <- sqrt(rowSums((bt[,1:3]/eta)^2))
                zgs[[j]] <- SRS*Mps[j]*(1+gamma)/4*(1/RCT-1/as[j])
                zgT <- zgT+zgs[[j]]
            }
###decide whether to repeat
        }
        if(max(abs(roemerT-roemerT0))<tol) break()
        roemerT0 <- roemerT
    }

    if(binarity & length(indC)>0){
        roemerT[indC] <- -roemerT[indC]*Mstar/Mps[imax]
        BT[indC,] <- -BT[indC,]*Mstar/Mps[imax]
        zgT[indC] <- zgs[[imax]][indC]*Mstar/Mps[imax]
    }

    RBT <- rowSums(BT[,1:3]^2)#au
    ##ignore the correction of vSB caused by astrometric offset
    vST <- out1$vSB+BT[,4:6]#relative velocity; au/yr
    VST2 <- rowSums(vST*vST)
    VSB2 <- rowSums(out1$vSB*out1$vSB)
...
#    if(binarity){
#        zgC <- SRS*Mstar*(1+gamma)/4*(1/RCT[[imax]][indC]-1/as[imax])#GR relativistic
#        rvgC <- zgC*CMPS#m/s
#        vSC <- out1$vSB[indC,]+BC[,4:6]#relative velocity; au/yr
#        VSC2 <- rowSums(vSC*vSC)
#        rvsC <- gamma*(VSC2-VSB2[indC])/Cauyr^2/2*CMPS#m/s
#        rvgsC <- rvgC+rvsC
#    }else{
#        rvsC <- rvgC <- rvgsC <- 0
#    }
###calibrate the radial velocity by accounting for astrometric bias
    drvT.du <-  0
    if(any(grepl('dra$',names(pars.kep))) & length(out1$astrometry)>0){
        obs0 <- unlist(out1$astrometry[out1$iref,c('ra','dec','parallax','pmra','pmdec','radial_velocity')])#reference astrometry
        dra <- pars.kep['dra']/cos(obs0['dec']/180*pi)/3.6e6
        if(!any(names(pars.kep)=='drv')){
            drv <- 0
        }else{
            drv <- pars.kep['drv']
        }
        obs <- obs0-c(dra,pars.kep['ddec']/3.6e6,pars.kep[c('dplx','dpmra','dpmdec')],drv)#corrected barycentric astrometry
        ast1 <- obs.lin.prop(obs0,t-out1$astrometry[out1$iref,'ref_epoch'])
        ast2 <- obs.lin.prop(obs,t-out1$astrometry[out1$iref,'ref_epoch'])
        dast <- ast2-ast1
        du <- t(outer(out1$pqu[,1],dast[,1]/180*pi*cos(obs['dec']/180*pi),'*'))+t(outer(out1$pqu[,2],dast[,2]/180*pi,'*'))
###add reflex motion
        uOB <- out1$uOB+du
        dau <- 1000/ast2[,'parallax']*pc2au
        duT <- du+BT[,1:3]/dau#change of direction in rad
#        uOT <- gen_CalUnit(out1$uOB+duT)
        uOT <- gen_CalUnit(-out1$rSO/pc2km+ast2[,c('x','y','z')]+BT[,1:3]/pc2au)

###add reflex motion to binary companion
        drvT.du <- rowSums(out1$vSO*duT*1e3)#m/s
        rvSOt <- rowSums(out1$vSO*uOT*1e3)#m/s
#        rvSOt <- out1$rvSO+drvT.du
    }
#    rvST <- ast2[,'radial_velocity']*1e3+BT[,6]*4.74047e3#m/s
#    rvST <- ast2[,'radial_velocity']*1e3+rowSums(BT[,4:6]*uOT)*4.74047e3#m/s;
    rv.kep <- rowSums(BT[,4:6]*uOT)*auyr2kms*1e3#m/s
    if(FALSE){
        pdf('test.pdf')
        aa <- read.table('../data/combined/HD128620/rvs/HD128620_PFS.vels',header=TRUE)
        ab <- read.table('../data/combined/HD128621/HD128621_PFS.vels',header=TRUE)
        plot(t,rv.kep,ylim=range(rv.kep,aa[,2],ab[,2]))
        points(aa[,1],aa[,2],col='red')
        points(ab[,1],ab[,2],col='blue')
        dev.off()
    }
    rv.sys <- rowSums(ast2[,c('vx','vy','vz')]*uOT)*auyr2kms*1e3
    rvST <- rv.sys+rv.kep
#    ZlocalT <- (1-out1$ZgsO)/(1+rvSOt/CMPS-out1$ZlO)-1
#stop()
    ZlocalT <- (1-out1$ZgsO)/(1+rvSOt/CMPS)-1
    ZremoteT <- (1+rvST/CMPS)/(1-rvgsT/CMPS)-1#ignore ZlT
    ZT <- (ZlocalT+1)*(ZremoteT+1)-1
#    cat('head(ZT)=',head(ZT),'\n')
#    cat('head(rvST)=',head(rvST),'\n')
#    cat('head(rv.kep)=',head(rv.kep),'\n')
    rv0 <- rep(0,length(tauT))
    if(!sim.kep){
        for(ins in out1$ins.rv){
            ind <- out1$ind.all$rv[[ins]]
            rv0[ind] <- pars.kep[paste0('rv0_',ins)]
        }
        if(length(out1$data.binary)>0){
            for(ins in names(out1$ind.binary)){
                ind <- out1$ind.all$binary[[ins]]
                rv0[ind] <- pars.kep[paste0('rv0b_',ins)]
            }
        }
    }
    Zlt <- rv.sys/4.74047*dau/Cauyr^2*((ast2[,'pmra']/206265e3)^2+(ast2[,'pmdec']/206265e3)^2)*(t-out1$astrometry[out1$iref,'ref_epoch'])/365.25#eq 18 of WE2014; this is required if not using proper emission time as time stamp; because it is TSB motion, we use t (or BJDtdb) instead of tauT
    if(FALSE){
###the shapiro term is too small and could be dropped
        xrho <- rowSums(out1$uSO*uOT)
        xrhoxvec <- sweep(uOT, MARGIN = 2, STATS = xrho, FUN = "*")
        Zsd <- -SRS*rowSums((out1$vSO/CMPS)*(uOT-xrhoxvec))/out1$RSO/(1+xrho)
    }else{
        Zsd <- 0
    }
#    Zm <- (ZlocalT+1)*(1+rvST/CMPS)/(1+rv0/CKMPS)-1#eq. 24 of Wright & Eastman 2014; however ZremoteT would be different for the template and epoch data, thus it should still be included in alphaCen
#    cat('max(rvgsT)-min(rvgsT)=',max(rvgsT)-min(rvgsT),'\n')
    Zm <- (ZlocalT+1)*(ZremoteT+1)/(1+rv0/CKMPS)-1#eq. 24 of Wright & Eastman 2014
    rvST0 <- obs['radial_velocity']*1e3+rowSums(BT[,4:6]*out1$uOB)*4.74047e3
    betaE <- sqrt(rowSums(out1$vSO*out1$vSO))/CMPS
    gammaE <- 1/sqrt(1-betaE^2)
    ZB <- gammaE*(1+rvSOt/CMPS)/(1-out1$ZgO)*(1+rvST0/CMPS)/(1+rvST/CMPS)-1-Zlt-Zsd#eq. 25 of Wright & Eastman 2014
    Ztrue <- (1+Zm)*(1+ZB)-1#eq. 25 of Wright & Eastman 2014
...
if(FALSE){
    ...
    pdf('test1.pdf')
    drvST <- rvST-rvST[1]
    plot(t,drvST,col='red')
    harps <- read.table('../data/combined/HD128620/rvs/HD128620_HARPS.dat')
    points(harps[,1],harps[,2])
    dev.off()
}
    return(list(rvkep=rv.kep,rvsys=rv.sys,rvgsT=rvgsT,rvgT=rvgT,rvsT=rvsT,tauT=tauT,roemerT=roemerT,drvT.du=drvT.du,EP=EP,ZT=ZT,Zm=Zm,ZB=ZB,ZlocalT=ZlocalT))
}

astrometry.bary <- function(pars.kep,tt=NULL,out1=out,bases=rep('natural',10),Ein=NULL,pa=FALSE){
###This is to give both absolute and relative astrometry if the absolute astrometry data is given
    data.astrometry <- out1$astrometry
    if(is.null(tt)){
#        tt <- out1$BJDtdb
        tt <- out1$tauT
        sim <- FALSE
    ...
    dt <- data.astrometry[out1$iref,1]-out1$tref#epoch offset between astrometry reference point and the RV reference point
#    if(is.null(tt)) tt <- data.astrometry[,'ref_epoch']
    DT <- tt - data.astrometry[out1$iref,1]#relative to astrometry reference point
    if(grepl('hgca',astro.type)){
        dt <- as.numeric(data.astrometry[2,c('epoch_ra','epoch_dec')])-out1$tref
        DT <- as.numeric(tt[1:2]-tt[3:4])
    }
    plx1 <- pmdec1 <- pmra1 <- rep(0,length(tt))
    if(FALSE){
        Np.kep <- length(grep('per|logP|lnP|^P',names(pars.kep)))
        pp <- extract.par(pars.kep,out1=out1,bases=bases)
        es <- pp$e
        Omegas <- pp$Omega
        ws <- pp$omega
        Incs <- pp$Inc
...
###model parallax and mean proper motion; only assumption: constant heliocentric velocity; reference Gaia epoch
    obs <- unlist(data.astrometry[out1$iref,c('ra','dec','parallax','pmra','pmdec','radial_velocity')])
...
####model of astrometry
astrometry.kepler <- function(pars.kep,tt=NULL,out1=out,bases=rep('natural',10),Pmin=0){
    if(is.null(tt)){
        sim <- FALSE
    }else{
        sim <- TRUE
    ...
    tmp <- list()
    if(length(out1$astrometry)>0){
        tmp$barycenter <- astrometry.bary(pars.kep=pars.kep,tt=tt,out1=out1,bases=bases)
    }
    if(any(names(out)=='rel')){
        tmp$rel <- astrometry.rel(pars.kep,tt=tt,out1=out1,bases=bases)$rel
    }
    if(length(out1$data.epoch)>0){
        for(jj in 1:length(out1$ins.epoch)){
            i <-  out1$ins.epoch[jj]
#            if(!sim) tt <- out1$data.epoch[[i]][,1]#JD or BJD
            if(!sim) tt <- out1$tauT[out1$ind.all$epoch1[[i]]]
            if(grepl('hip',i)){
                band <- 'Hp'
            }else{
                band <- 'G'
...
####calculate the dra and ddec relative to the Gaia DR3 reference epoch
            if(!grepl('hip|TYC',i)){
                if(!sim){
                    plx.ra <- out1$pf[out1$ind.all$epoch1[[i]],'ra']
                    plx.dec <- out1$pf[out1$ind.all$epoch1[[i]],'dec']
                    bary <- tmp$barycenter[out1$ind.all$epoch1[[i]],]
                }else{
                    plx.ra <- out1$pf[,'ra']
                    plx.dec <- out1$pf[,'dec']
                    bary <- tmp$barycenter
...
#                dt <- (tt-out1$data.epoch[[i]][1,1])/365.25#year
#                dt.yr <- (tt-out1$astrometry[out1$iref,'ref_epoch'])/365.25#year
##astrometric parameter bias contribution to offsets
##dra/ddec is the offset relative to the reference position given by reference astrometry
#                dec1 <- out1$astrometry[out1$iref,'dec']+ddec1/3.6e6#deg
#                ra1 <- out1$astrometry[out1$iref,'ra']+dra1/3.6e6/cos(dec1/180*pi)#deg
#                save(list=ls(all=TRUE),file='test1.Robj')
                dra <- (bary[,'ra']-out1$astrometry[out1$iref,'ra'])*3.6e6*cos(out1$astrometry[out1$iref,'dec']/180*pi)+plx.ra*bary[,'parallax']#already considered perspective acceleration
                ddec <- (bary[,'dec']-out1$astrometry[out1$iref,'dec'])*3.6e6+plx.dec*bary[,'parallax']
#                ra <- tmp$barycenter[,'ra']
#                dec <- tmp$barycenter[,'dec']
                tmp$epoch[[i]] <- data.frame(dra=tmp$epoch[[i]][,'dra']+dra,ddec=tmp$epoch[[i]][,'ddec']+ddec)
#                plot(ra1,dec1,xlim=range(ra1,ra),ylim=range(dec1,dec))
#                points(ra,dec,col='red')
                ##                points(out1$astrometry[out1$iref,'ra'],out1$astrometry[out1$iref,'dec'],col='blue')
#                cat('dra=',head(tmp$epoch[[i]][,'dra']),'\n')
#                cat('ddec=',head(tmp$epoch[[i]][,'ddec']),'\n')
#                cat('dra.obs=',head(out1$data.epoch$ALMA[,'dra']),'\n')
#                cat('ddec.obs=',head(out1$data.epoch$ALMA[,'ddec']),'\n')
#
#                if(!sim){
                if(FALSE){
#                if(TRUE){
                pdf('test.pdf')
                plot(tmp$epoch[[i]][,1],tmp$epoch[[i]][,2],xlim=range(dra,out1$data.epoch$ALMA[,'dra'],dra+tmp$epoch[[i]][,'dra']),ylim=range(ddec,out1$data.epoch$ALMA[,'ddec'],ddec+tmp$epoch[[i]][,'ddec']))
                points(out1$data.epoch$ALMA[,'dra'],out1$data.epoch$ALMA[,'ddec'],col='red')
                dev.off()
                }
            }
        }
    ...
    if(length(out1$gost)>0){
        reflex <- astrometry.epoch(pars.kep,out1=out1,tt=out1$gost[,'BJD'],bases=bases,band='G')$epoch
        obs0 <- unlist(tmp$barycenter[out1$ind.all$catalog[[out1$iref]],])
#        bary <- obs.lin.prop(obs0,t=out1$gost[,'BJD']-out1$astrometry[out1$iref,'ref_epoch'])
        bary <- obs.lin.prop(obs0,t=out1$gost[,'BJD']-out1$astrometry[out1$iref,'ref_epoch'],PA=FALSE)
        dec <- bary[,'dec']+reflex[,'ddec']/3.6e6#deg
        dra <- (bary[,'ra']-out1$astrometry[out1$iref,'ra'])*cos(dec/180*pi)*3.6e6+reflex[,'dra']#mas
        ddec <- (dec-out1$astrometry[out1$iref,'dec'])*3.6e6#mas
        gabs <- dra*sin(out1$gost[,'psi'])+ddec*cos(out1$gost[,'psi'])+(bary[,'parallax']+reflex[,'dplx'])*out1$gost[,'parf']#mas
#        gabs <- dra*sin(out1$gost[,'psi'])+ddec*cos(out1$gost[,'psi'])+bary[,'parallax']*out1$gost[,'parf']
        dabs <- list()
        binarycats <- cats <- c()
        for(k in 1:length(out1$cats)){
            data <- data.frame(gabs=gabs[out1$cat.ind[[k]]],a1=out1$a1[[k]],a2=out1$a2[[k]],a3=out1$a3[[k]],a4=out1$a4[[k]],a5=out1$a5[[k]])
            if(out1$cats[k]=='GDR1'){
                ast <- lm(gabs~a1+a2+0,data=data)$coefficients
                res <- out1$astro.gost[k,1:2]-ast
                res <- c(res,rep(0,3))
            }else if(out1$cats[k]=='TYC'){
                t <- out1$astrometry[out1$ityc,'ref_epoch']
                reflex1 <- astrometry.epoch(pars.kep,out1=out1,tt=t,bases=bases,band='H')$epoch
                bary1 <- obs.lin.prop(obs0,t=t-out1$astrometry[out1$iref,'ref_epoch'])
                dastro <- AstroDiff(bary1[1,],out1$astrometry[out1$ityc,])
                res <- dastro[c('ra','dec')]
                if(is.na(out1$astrometry[out1$ityc,'parallax'])){
                ...
                dabs[[out1$cats[k]]] <- fit$residuals
                res <- out1$astro.gost[k,]-ast
            }
            cats <- rbind(cats,res)
        }
        tmp$cats <- cats
        tmp$dabs <- dabs
        if(length(out1$binary$gost)>0){
            ...
            reflex2 <- astrometry.epoch(pars.kep2,out1=out1,tt=out1$gost[,'BJD'],bases=bases,band='G')$epoch
            ...
            for(k in 1:length(out$binary$cats)){
                ...
                if(out$binary$cats[k]=='GDR1'){
                    ...
                    reflex1 <- astrometry.epoch(pars.kep2,out1=out1,tt=t,bases=bases,band='H')$epoch
...
RV.kepler <- function(pars.kep,tt=NULL,out1=out,prior.kep=prior.type,injection=FALSE,kep.only=FALSE,noise.only=FALSE,bases=rep('natural',10),rv.pm=0,nqp=NULL){
    if(is.null(tt)){
        sim.kep <- FALSE
    }else{
        sim.kep <- TRUE
    ...
    if(length(out1$data.binary)>0) binarity <- TRUE
    if(!sim.kep){
        if(any(names(out)=='BJDtdb') & dtype=='raw'){
            tt <- out1$BJDtdb
        }else{
            tt <- out1$tiall[,1]
        }
...
#    tt <- tt-tmin
    ...
    EC <- rvgC <- rvsC <- drvC <- drvkepC <- drvT.du <- 0
    tauT <- tt
...
###add relativistic effects
        ...
        drvbary <- drvkep <- drvT <- rep(0,length(tt))
        tt0 <- c()
...
#                save(list=ls(all=TRUE),file='test0.Robj')
                ...
                rvsP <- tmp$rvsT
                drvT.du <- tmp$drvT.du
#                drvT <- drvT.du+rvgP+rvsP+tmp$drvT.du#account for relativistic and astroemtric effect
#                drvT <- tmp$ZT*CMPS#-out1$rvs
...
#                cat('sd(rv.data-drvT)=',sd(out1$PFS$data[,2]-drvT[out$ind.all$rv$PFS]),'\n')
                ...
                for(h in 1:Np.kep){
                    m <- (pp$Mo[h]+2*pi*(tt-out1$tref)/pp$P[h])%%(2*pi)#mean anomaly
                    if(prior.kep!='e0'){
                    ...
                    drvT <- drvT+pp$K[h]*(cos(pp$omega[h]+T)+pp$e[h]*cos(pp$omega[h]))
                }
                drvkep <- drvT
            }
        }
...
###planet veloctiy
                        ...
                        if(!sim.kep){
                            m <- (pp$Mo[i]+2*pi*(out1$rvc[[paste0('p',i)]][,1]-out1$tref)/pp$P[i])%%(2*pi)#mean anomaly
                        }else{
                            m <- (pp$Mo[i]+2*pi*(tt-out1$tref)/pp$P[i])%%(2*pi)
                        }
                        if(prior.kep!='e0'){
...
###add moon perturbation
                        if(out1$Nm>0 & out1$moon[[i]][1]>0){
                            inds <- (out1$moon[[i]][2]:out1$moon[[i]][3])+Np.kep
###moon mass
                                        #                                eta <- (msini$ms+Mp)/Mp
                            for(k3 in inds){
                                if(!sim.kep){
                                    m <- (pp$Mo[k3]+2*pi*(out1$rvc[[paste0('p',i)]][,1]-out1$tref)/pp$P[k3])%%(2*pi)#mean anomaly
                                }else{
                                    m <- (pp$Mo[k3]+2*pi*(tt-out1$tref)/pp$P[k3])%%(2*pi)
                                }
                                if(prior.kep!='e0'){
...
###calculate noise component
    ...
    if(sim.kep){
        ...
        Nins <- length(out1$ins.rv)
    }
    if(out1$Nrv>0){
        for(j2 in 1:Nins){
###assign parameters and data
            ii <- out1$ind.all$rv[[out1$ins.rv[j2]]]
            ins <- out1$ins.rv[j2]
            if(is.null(nqp.all)){
                nqp <- out1[[ins]]$noise$nqp
            }else{
                nqp <- nqp.all[[ins]]
            }
            if(!kep.only){
                ...
                b <- 0
                inds <- out1$ind.all$rv[[ins]]
                if(offset) b <- pars.kep[paste0('b_',ins)]
                if(!sim.kep){
                    trend <- cal.trend(a=at,b=b,t=(tt[inds]-out1$tref)/time.unit)
                    drvT[inds] <- drvT[inds]+trend
                }else{
                    trend <- cal.trend(a=at,b=b,t=(tsim-out1$tref)/time.unit)
                    drvT <- drvT+trend
                }

###check whether PM-induced RV trend is subtracted; perspective acceleration
                if(grepl('VLC|LC|CFHT',out1$ins.rv[j2]) & length(out1$astrometry)>0){
                    t3 <- out[[out1$ins.rv[j2]]]$RV[,1]-out1$astrometry[out1$iref,1]
                    obs0 <- unlist(out1$astrometry[out1$iref,c('ra','dec','parallax','pmra','pmdec','radial_velocity')])#reference astrometry
                    tmp <- obs.lin.prop(obs0,t3)
                    rv.pm <- (tmp[,'radial_velocity']-tmp[1,'radial_velocity'])*1e3#m/s
                    drvT[inds] <- drvT[inds]+rv.pm
                }
                ...
                if(nqp[1]>0){
                    ...
                    if(length(ind)>0){
                        ...
                        drvT[inds] <- drvT[inds] + tmp
                    }
                }
            }
        }
    }
    return(list(tt=tt,E=Ein,rv=drvT,rvkep=drvkep,rvbary=drvbary,rvg=rvgP,drvT=drvT,drvT.du=drvT.du,rvs=rvsP,rvm=rvm,tauT=tauT))
}
...
#likelihood
loglikelihood <- function(pars,out1=out,bases=rep('natural',10),RVonly=FALSE,nqp=NULL,prediction=FALSE,indrel=NULL,verbose=FALSE){
    if(verbose){
        cat('\n')
    ...
    if(is.null(indrel)){
        indrel <- out1$indrel
        typerel <- out1$typerel
    }
    pp <- extract.par(pars,out1=out1,bases=bases)
    nqp.all <- nqp
    res.all <- list()
    if(out1$Nrv>0 | out1$Nrvc>0 | length(out1$tiall)>0){
        rvp  <-  RV.kepler(pars.kep=pars,out1=out1,bases=bases,nqp=nqp)
        RV.kep <- rvp$rv
        out1$tauT <- rvp$tauT
    }
    Np <- length(grep('omega',names(pars)))
    epoch <- barycenter <- ref <- rel <- hg <- NULL
#    if((out1$Nastro>0 | any(names(out)=='rel') | length(out1$data.ref)>0 | length(out1$data.epoch)>0 | length(out1$gost)>0) & !RVonly){
    if((out1$Nastro>0 | any(names(out)=='rel') | length(out1$data.epoch)>0 | length(out1$gost)>0) & !RVonly){
        astro <- astrometry.kepler(pars.kep=pars,out1=out1,bases=bases,Pmin=Tmin)
...
#par(mfrow=c(4,4))
    ...
    if(out1$Nrv>0){
        for(k3 in 1:length(ins)){
            trv <- out[[out1$ins.rv[k3]]]$RV[,1]
            rv.data <- out[[out1$ins.rv[k3]]]$RV[,2]
            erv <- out[[out1$ins.rv[k3]]]$RV[,3]
            inds <- out$ind.all$rv[[ins[k3]]]
            rv.model <- rv.kep <- RV.kep[inds]
            if(is.null(nqp.all)){
                nqp <- out1[[out1$ins.rv[k3]]]$noise$nqp
            }else{
                nqp <- nqp.all[[out1$ins.rv[k3]]]
            }

            s <- pars[paste0('J_',out1$ins.rv[k3])]
            if(prediction) res.all[[out1$ins.rv[k3]]][['kep']] <- rv.kep
            if(prediction) res.all[[out1$ins.rv[k3]]][['res1']] <- rv.data-rv.kep
            if(nqp[2]>0 | nqp[3]>0){
                rv.arma <- arma(t=trv,ymodel=rv.kep,ydata=rv.data,pars=pars,set=out1$ins.rv[k3],p=nqp[3],q=nqp[2])
                rv.model <- rv.model +rv.arma
                if(prediction) res.all[[out1$ins.rv[k3]]][['red']] <- rv.arma
            }
            if(prediction) res.all[[out1$ins.rv[k3]]][['res2']] <- rv.data-rv.model
            ll <- sum(dnorm(rv.data,mean=rv.model,sd=sqrt(erv^2+s^2),log=T))
            logLike <-  logLike +ll
#
            if(verbose) cat('ll for',ins[k3],'RV=',ll,'\n')
        }
    }
    ll.astro <- 0
    if(any(names(out)=='rel')){
        for(j in 1:Np){
            if(length(indrel[[j]])>0){
                n <- paste0('p',j)
                inss <- names(out1$rel[[n]])
                inss <- inss[inss!='tot']
                for(i in inss){
                    ll <- 0
                    if(typerel[[n]][[i]]=='rd'){
                        res <- cbind(out1$rel[[n]][[i]][,'dra']-rel$dra[[n]][[i]],out1$rel[[n]][[i]][,'ddec']-rel$ddec[[n]][[i]])
                        erel <- cbind(out1$rel[[n]][[i]][,'era'],out1$rel[[n]][[i]][,'edec'])
                        if(any(colnames(out1$rel[[n]][[i]])=='dpmra')){
                            res <- cbind(res,out1$rel[[n]][[i]][,'dpmra']-rel$dpmra[[n]][[i]],out1$rel[[n]][[i]][,'dpmdec']-rel$dpmdec[[n]][[i]])
                            erel <- cbind(erel,out1$rel[[n]][[i]][,'epmra'],out1$rel[[n]][[i]][,'epmdec'])
                        }
                        if(any(names(out)=='cov')){
                            cov.rel <- out1$cov[[n]][[i]]
                        }
                        ##                        cov <- matrix(c(erel[k,1]^2,out1$rel[[n]][[i]][k,'cov'],out1$rel[[n]][[i]][k,'cov'],erel[k,2]^2),nrow=2)
                    }else{
                        ps <- rd2ps(rel$dra[[n]][[i]],0,rel$ddec[[n]][[i]],0)#model
                        res <- cbind(out1$rel[[n]][[i]][,'sep']-ps[,'sep'],out1$rel[[n]][[i]][,'pa']-ps[,'pa'])#residual
                        erel <- cbind(out1$rel[[n]][[i]][,'esep'],out1$rel[[n]][[i]][,'epa'])#error
...
#            dpmdec <- epoch[[i]][,'dpmdec']
            ...
            if(grepl('hip',i)){
                dastro <- AstroDiff(out1$astrometry[out1$ihip,],barycenter[out1$ind.all$catalog$hip,])
##contribution of barycenter-offset to target astrometry
                ...
                if(i=='hip1'){
                    ...
                    if(is.null(out1$cov.epoch[[i]])){
                        ll <- ll+sum(dnorm(res, mean=0, sd=data.epoch[,'ev'], log=TRUE))
                    }else{
                        ll <- ll+mvtnorm::dmvnorm(res,mean=rep(0,length(res)), sigma=out1$cov.epoch[[i]]*s, log=TRUE)
                    }
                }
                if(i=='hip2'){
                ...
                dra.res <- out1$data.epoch[[i]][,'dra']-dra
                ddec.res <- out1$data.epoch[[i]][,'ddec']-ddec
                res <- cbind(dra.res,ddec.res)
#                cat('length(dra.res)=',length(dra.res),'\n')
#                cat('length(ddec.res)=',length(ddec.res),'\n')
                ll.epoch <- sum(dnorm(as.numeric(res),mean=0,sd=sqrt(c(out1$data.epoch[[i]][,'era'],out1$data.epoch[[i]][,'edec'])^2+s^2),log=TRUE))
...
####Gaia GOST fit
    if(!is.null(out1$gost)){
        ll.gost <- 0
        s <- 0
        nast <- length(out1$astro.index)
        for(k in 1:nast){
            j <- out1$astro.index[k]
            s <- 1
            if(grepl('GDR',toupper(out1$cats[k]))){
...
#            cat('s=',s,'\n')
            if(grepl('TYC',toupper(out1$cats[k]))) s <- pars['J_TYC']
            if(grepl('HIP',toupper(out1$cats[k]))) s <- pars['J_hip']
            if(prediction){
                res.all[[out1$cats[k]]] <- astro$cats[k,]
            }
            if(out1$cats[k]=='TYC'){
                ll <- mvtnorm::dmvnorm(astro$cats[k,1:2], mean=rep(0,2), sigma=out1$cov.astro[1:2,1:2,j]*s, log=TRUE)
            }else{
                ll <- mvtnorm::dmvnorm(astro$cats[k,], mean=rep(0,5), sigma=out1$cov.astro[1:5,1:5,j]*s, log=TRUE)
            }
            ll.gost <- ll.gost+ll
        }
        ...
        if(verbose){
            ...
            smb <- 0
            ys <- c()
            p <- obs.lin.prop(out1$astrometry[out1$iref,],t=out1$astrometry[,'ref_epoch']-out1$astrometry[out1$iref,'ref_epoch'])
            for(k in 1:nast){
                y <- t(t(as.numeric(AstroDiff(out1$astrometry[k,],p[k,])[1:5])))
                ys <- cbind(ys,y)
                j <- out1$astro.index[k]
                sm0 <- solve(out1$cov.astro[1:5,1:5,j])
                sm <- sm+sm0
                smb <- smb + sm0%*%y
            }
            C0b <- solve(sm)
            ...
            for(k in 1:nast){
                j <- out1$astro.index[k]
                ll0.gost <- ll0.gost+mvtnorm::dmvnorm(ys[,k], mean=beta0b, sigma=out1$cov.astro[1:5,1:5,j], log=TRUE)
            }
            cat('ll for gost astrometry=',ll.gost,'\n')
            cat('ll.gost-ll0.gost=',ll.gost-ll0.gost,'\n')
        }
        ll.ruwe <- 0
        if(ruweDR==3){
...
####timing data and model
    if(!is.null(out1$timing)){
        if(verbose) ll0.time <- 0
        ll.time <- 0
        for(n1 in names(out1$timing)){
            for(n2 in names(out1$timing[[n1]])){
                index <- out1$ind.all$timing[[paste0(n1,'_',n2)]]
                if(!grepl('pulsation|eb',n2)){
                    ts <- rvp$tt[index]
                    lte  <-  (rvp$tt[index]-rvp$tauT[index])*24*60#min
                    ip <- as.integer(gsub('p','',n1))
                    p <- pp$P[ip]#day
                    dt <- out1$timing[[n1]][[n2]][,1]-out1$ttv0
                    if(grepl('occult',n2)) dt <- dt+p/2
                    ttv <- (dt-round(dt/p)*p)*24*60
                    ettv <- out1$timing[[n1]][[n2]][,2]*24*60
...
#                    dt.yr <- (out1$timing[[n1]][[n2]][,1]-out1$timing[[n1]][[n2]][1,1])/365.25
                    dt.yr <- (out1$timing[[n1]][[n2]][,1]-out1$tref)/365.25
                    ...
                    ll1 <- sum(dnorm(out1$timing[[n1]][[n2]][,'dt'],mean=dt.model,sd=out1$timing[[n1]][[n2]][,'et'],log=TRUE))
                    ll.time <- ll.time+ll1
                }
            }
        }
...
#####companion's RV
    if(out1$Nrvc>0){
        for(j in 1:Np){
            if(any(j==out1$Irvc)){
                n <- paste0('p',j)
                res <- out1$rvc[[n]][,2]-rvp$rvc[[n]]
#                cat('out1$rvc[[n]]=',out1$rvc[[n]][,2],'\n')
#                cat('rvp$rvc[[n]]=',rvp$rvc[[n]],'\n')
#                cat('res=',res,'\n')
#                cat('ll=',ll,'\n')
                ll <- sum(dnorm(res, mean=0, sd=out1$rvc[[n]][,3], log=TRUE))
                logLike <- logLike+ll
...
####binary
   if(length(out1$data.binary)>0){
       Nins <- length(out1$ins.binary)
       for(j in 1:Nins){
           ins <- out1$ins.binary[j]
#           res <- out1$data.binary[out1$ind.binary[[ins]],2]-rvp$drvC.du[out1$ind.binary[[ins]]]
           inds <- out1$ind.all$binary[[ins]]
           indB <- out1$ind.binary[[ins]]
           res <- out1$data.binary[indB,2]-rvp$rv[inds]
           jitter <- pars[paste0('Jb_',ins)]
           ll <- sum(dnorm(res, mean=0, sd=sqrt(out1$data.binary[indB,3]^2+jitter^2), log=TRUE))
           logLike <- logLike+ll
#            save(list=ls(all=TRUE),file='test3.Robj')
           if(verbose) cat('ll for',ins,'binary=',ll,'\n')
       }
    }
...
cal.residual <- function(pars,out1=out,bases=rep('natural',10),nqp=NULL){
    nqp.all <- nqp
    RV.all  <-  RV.kepler(pars.kep=pars,out1=out1,bases=bases,nqp=nqp)$rv
    tmp <- RV.kepler(pars.kep=pars,out1=out1,kep.only=TRUE,bases=bases,nqp=nqp)
    if(dtype!='raw'){
        RV.sig <- RV.kep  <-  tmp$rv
        RV.bary <- rep(0,length(RV.kep))
    }else{
        RV.sig <- tmp$rv
        RV.kep <- tmp$rvkep
        RV.bary <- tmp$rvbary
    ...
    rvs <- ervs <- trend <- arma1 <- all <- noise <- signal <- residual.all <- residual.sigtrend <- residual.noise <- residual.sig <- rep(NA,length(RV.sig))
    if(out1$Nrv>0){
        for(j3 in 1:2){
            if(j3==1){
                inss <- out1$ins.rv
                ...
                inss <- out1$ins.binary
                binarity <- TRUE
            }
            for(ins in inss){
                if(binarity){
                    inds <- out1$ind.all$binary[[ins]]
                    rv.data <- out1$data.binary[out1$data.binary$ins==ins,2]
                    erv <- out1$data.binary[out1$data.binary$ins==ins,3]
                }else{
                    inds <- out1$ind.all$rv[[ins]]
                    rv.data <- out1[[ins]]$RV[,2]
                    erv <- out1[[ins]]$RV[,3]
                }
#                tauTs[inds] <- out1$tauT[inds]
                rvs[inds] <- rv.data
                ervs[inds] <- erv
                trv <- out1$tauT[inds]
                rv.all <- RV.all[inds]
                ...
                if((nqp[2]>0 | nqp[3]>0)){
                    rv.arma <- arma(t=trv,ymodel=rv.all,ydata=rv.data,pars=pars,set=ins,p=nqp[3],q=nqp[2])
                    if(FALSE){
                ...
                res.kep <- rv.data-rv.kep
                res.sigtrend <- rv.data-rv.sig-rv.trend
                rv.noise <- rv.all-rv.sig
                res.noise <- rv.data-rv.noise
                residual.all[inds] <- res.all
                residual.sig[inds] <- res.sig
                residual.sigtrend[inds] <- res.sigtrend
                residual.noise[inds] <- res.noise
                signal[inds] <- rv.sig
                noise[inds] <- rv.noise
                all[inds] <- rv.all
                arma1[inds] <- rv.arma
    ...
    list(rvs,ervs,res.kep=res.kep,res.sig=residual.sig,res.sigtrend=residual.sigtrend,res.all=residual.all,res.noise=residual.noise,rv.bary=RV.bary,rv.sig=RV.sig,rv.kep=RV.kep,rv.signal=signal,rv.noise=noise,rv.all=all,rv.arma=arma1,rv.trend=trend)
}
...
prior.func <- function(pars,out1=out,bases='natural',nqp=NULL){
    nqp.all  <- nqp
    logprior <- 0
    pp <- extract.par(pars,out1=out1,bases=bases)
#    Np <- length(grep('per',names(pars)))
    ...
    if(Ntot>0){
        for(k in 1:Ntot){
            ...
            if(length(indM)>0){
                Mstar <- pars['Mstar']
                if(priorf==1) mprior <- dnorm(Mstar,mean=out1$Mstar,sd=out1$eMstar,log=TRUE)
#                cat('Mstar=',Mstar,';mprior=',mprior,';out1$Mstar',out1$Mstar,';out1$eMstar',out1$eMstar,'\n')
            }else{
                Mstar <- out1$Mstar
            }
...
#                    cat('logP.prior=',logP.prior,'\n')
                    ...
                    lp <- dnorm(mp,out1$mp[1,k],out1$mp[2,k],log=TRUE)
...
####noise model
    if(out1$Nrv>0){
        for(i1 in 1:length(out1$ins.rv)){
            if(is.null(nqp.all)){
                nqp <- out[[out1$ins.rv[i1]]]$noise$nqp
            }else{
                nqp <- nqp.all[[out1$ins.rv[i1]]]
            }
...
#        lp <- lp+dnorm(out1$astrometry[out1$iref,'parallax']-pars['dplx'],0.68,0.05,log=TRUE)
        plx <- out1$astrometry[out1$iref,'parallax']-pars['dplx']
        ...
        out1$Mstar <- pmfun.spec(plx)
        out1$eMstar <- pemfun.spec(plx)
    }
...
#        lp <- lp+dnorm(out1$astrometry[out1$iref,'parallax']-pars['dplx'],0.68,0.05,log=TRUE)
    ...
    if(target=='HD253754'){
        lp <- lp+dnorm(out1$astrometry[out1$iref,'parallax']-pars['dplx'],1.1,0.3,log=TRUE)
    }
    if(target=='CPD-632495'){
        bary <- astrometry.bary(pars.kep=pars,out1=out1,tt=2400000.5+55000,bases=bases)
...
####Jitter prior
    if(length(out1$rel.ins)>0){
        js <- unlist(sapply(out1$rel.ins, function(i) pars[grep(paste0('J_.+',i),names(pars))]))
        lp <- lp + sum(dnorm(js,1,0.1,log=TRUE))
#        cat('prior for ',out1$rel.ins,':',dnorm(js,1,0.1,log=TRUE),'\n')
    }
    if(length(out1$astro.ins)>0){
        js <- unlist(sapply(out1$astro.ins, function(i) pars[grep(paste0('J_',i),names(pars))]))
        lp <- lp + sum(dnorm(js,1,0.1,log=TRUE))
#        cat('prior for ',out1$astro.ins,':',dnorm(js,1,0.1,log=TRUE),'\n')
    }
####total
...
posterior <- function(param,out1=out,tem=1,bases='natural',RVonly=FALSE,verbose=FALSE){

    llike <- loglikelihood(param,out1=out1,bases=bases,RVonly=RVonly,verbose=verbose)
    pr <- prior.func(param,bases=bases,out1=out1)
    post <- llike*tem+pr
...
###run MH algorithm
run.metropolis.MCMC <- function(startvalue,cov.start,iterations,out1=out,n0=10,verbose=FALSE,tem=1,bases=rep('natural',10),RVonly=FALSE){
    ...
    chain[1,]<- startvalue
    mu1 <- chain[1,]
    logpost.out = posterior(chain[1,],out1=out1,tem=tem,bases=bases,RVonly=RVonly)
...
#            cat('i=',i,';tem=',tem,';diag(cov.adapt)=',diag(cov.adapt),';')
            ...
            if(Np>=1){
                pars.now <- chain[i+1,]
                pp <- extract.par(pars.now,out1=out1,bases=bases)
                Ps <- pp$P
                Ks  <-  pp$K
                es  <-  pp$e
                cat('; period= ',Ps,'; K=', Ks)
