...
if(length(args)>0){
    comptype <- as.character(args[1])
    Nmc <- Nsamp <- as.integer(args[2])
    star <- as.character(args[3])
}else{
    comptype <- 'companion'
    Nmc <- Nsamp <- 0
#    star <- 'HD42581'
...
###plotting settings
    j1 <- i1
    imaging <- FALSE
    if(grepl('hg123\\+',fs[i1])) imaging <- TRUE
    if(plotf){
...
###simulation for ylim
        ...
        tsim <- out$tsim
        rvs <- RV.kepler(pars.kep=par.opt,tt=out$tsim,kep.only=TRUE,nqp=nqp)$rv
        for(i3 in 1:Nset){
...
#        legend('topleft',legend=c(instr,'Hipparcos','Gaia','Best fit','Barycenter'),col=c(cols[1:Nw0],ell.col,fit.opt,'black'),lty=c(rep(NA,Nw0+2),1,NA),pch=c(rep(20,Nw0),ell.pch,NA,3),xpd=NA,inset=inset,bty='n',title=sn,lwd=c(rep(NA,Nw0+2),line.size,NA),pt.cex=c(rep(1,Nw0),rep(1,2),NA,1),y.intersp=1.1)
        if(!imaging){
            legend('topleft',legend=c(instr,'Hipparcos','Gaia','Best fit','Barycenter'),col=c(cols[1:Nw0],ell.col,fit.opt[1],'black'),lty=c(rep(NA,Nw0+2),1),pch=c(rep(20,Nw0),rep(ell.pch,2),NA,3),xpd=NA,inset=inset,bty='n',title=sn,lwd=c(rep(NA,Nw0+2),line.size,NA),pt.cex=c(rep(1,Nw0),rep(1,2),NA,1),y.intersp=1.1)
        }else{
...
#            Nrel <- length(out$rel)
            Nrel <- 1
            legend('topleft',legend=c(instr,'Hipparcos','Gaia',inss,rep('Best fit',Nrel),'Barycenter'),col=c(cols[1:Nw0],ell.col,col.rel,fit.opt[1:Nrel],'black'),lty=c(rep(NA,Nw0+2+Nins),rep(1,Nrel),NA),pch=c(rep(20,Nw0),rep(ell.pch,2),rep(rel.pch,Nins),rep(NA,Nrel),3),xpd=NA,inset=inset,bty='n',title=sn,lwd=c(rep(NA,Nw0+2+Nins),rep(line.size,Nrel),NA),pt.cex=c(rep(1,Nw0+2+Nins),rep(NA,Nrel),1),y.intersp=1.1)
        }
    }
    ...
    if(plotf){
        ...
        lines(tt%%24e5-tmin%%24e5,RV.kepler(pars.kep=par.opt,tt=tt,kep.only=TRUE,nqp=nqp)$rv,col=fit.opt[1],lwd=line.size)
...
###residual plot
        ...
        for(j2 in 1:Nset){
            ...
            ey <- erv.list[[j2]]
            if(j2==1){
                plot(x,y,xlab='',ylab='',ylim=ylim,xlim=xlim,col=tcol(cols[j2],50),pch=20,cex=size,xaxt='n',yaxt='n')
                                        #            axis(1,tcl = 0.5)
                ...
                points(x,y,col=cols[j2],pch=20,cex=1)
            }
            arrows(x,y-ey,x,y+ey,length=0.05,angle=90,code=3,col=tcol(cols[j2],50))
        }
                                        #    par(mgp=c(3,1,0),xpd=NA)
        if(i1==1) mtext(side=1,text=paste0('JD-',tmin%%24e5+24e5),xpd=NA,line=2,cex=1.2*size)
        mtext(side=2,text=expression('O-C [m/s]'),line=1.5,xpd=NA,cex=1.2*size)
        abline(h=0,lty=2,lwd=2)
    }
####astrometry
    if(imaging){
        source('reflex_motion_imaging.R')
    }else{
