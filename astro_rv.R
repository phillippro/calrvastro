...
ind <- which(targets==star)
if(length(ind)>0){
fs <- fs[ind]
targets0 <- targets <- stars <- targets[ind]
}else{
stop('no file is found!')
}
plotf <- TRUE
etaH <- etaG <- 1
#fs <- sort(fs)
...
if(plotf){
  fpdf <- paste0(astrotype,'/',star,'_N',Nmc,'_fit.pdf')
    cat('output:\n',fpdf,'\n')
    pdf(fpdf,12,3*Nsig)
}
fit.col <- tcol('red',90)
...
###simulation for ylim
    ...
    t3 <- seq(min(tall),max(tall),length.out=100)
