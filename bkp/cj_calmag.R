...
tab <- read.table('companion_mpa.txt',header=TRUE)
cc <- colnames(tab)
...
###get the separation at 2023 and the closest and widest separations for each companion
        if(length(ind1)>0){
            ...
            if(is.na(lum)){
                ...
                mstars <- c(mstars,tab[j,'mstar'])
                emstars <- c(emstars,tab[j,'emstar'])
            }
        }else{
            cat('mlr!\n')
            lum <- m2l.eker15(tab[j,'mstar'])
        ...
        ss <- rbind(ss,c(tab[j,1],mstar,emstar,as.numeric(lum),as.numeric(plx)))
        alphas <- tab[j,c('rho','e_rho','E_rho')]*1e3#mas
        tmp <- rbind(tmp,c(as.numeric(tab[j,c('m','e_m','E_m')]),as.numeric(tab[j,c('p','e_p','E_p')])/365.25,as.numeric(alphas)))
}
tmp <- data.frame(cbind(ss,tmp))
