...
#    f = 1./(xdif*ofac) + c(1:nout)/(ofac*xdif)
    ...
    w <- 1/err2/W
    bigY <- sum(w*y)  # Eq. (7)
    chi2s <- p <- rep(NA,length(omegas))
    pars <- array(NA,dim=c(length(omegas),3))
...
####according to A4 of Zechmeister & Kursteret 2009
        a <- (YC*SS-YS*CS)/bigD
        b <- (YS*CC-YC*CS)/bigD
        c <- bigY-a*bigC-b*bigS
        pars[i,] <- c(a,b,c)
    }
    chi2.ref <- YY*W
    chi2s <- chi2.ref-p*chi2.ref
    N <- length(y)
 # An ad-hoc estimate of the number of independent frequencies (ZK_09 Eq. 24)
    ...
    names(par.opt) <- c('A','B','gamma')
    lnBFs <- 0.5*(chi2.ref-chi2s)-1.5*log(N)
    yp <- par.opt[1]*cos(omega.opt*t)+par.opt[2]*sin(omega.opt*t)+par.opt[3]
    res <- y-yp
...
if(FALSE){
    ...
    par.full <- c(as.list(coef(out)),par.fix)
    yp <- gls.model(t,par.full)
    ll <- sum(dnorm(y,mean=yp,sd=err,log=T))
    tmp <- lm(y~t, data=df)
    ll0 <- sum(dnorm(y,mean=mean(y),sd=err,log=T))
    lnBFs <- ll-ll0-1.5*log(N)
    res <- y-yp
    yp <- gls.model(t,par.full)
}
#    cat('level:',level,'\n')
    ...
    ps <- 1/f[inds[1:5]]
    power.opt <- power[inds[1:5]]
    return(list(P=1/f, power=power, pvalue=pp, sig.level=level,res=res,par.opt=par.opt,ps=ps,power.opt=power.opt,lnBFs=lnBFs))
}
...
#    f <- seq(fmin,fmax,by=step)*unit
    ...
    bigY <- sum(w*y)  # Eq. (7)
    chi2s <- p <- rep(NA,length(omegas))
    pars <- array(NA,dim=c(length(omegas),4))
    bigT <- sum(w*t)
    YY.hat <- sum(w*y^2)
    YT.hat <- sum(w*y*t)
    TT.hat <- sum(w*t^2)
...
#    FAP <- c(0.317,0.046,3e-3)#significance level of FAP
    ...
    ps <- 1/f[inds[1:5]]
    power.opt <- power[inds[1:5]]
    return(list(P=unit/f, power=power, pvalue=pp, sig.level=level,Popt=P[ind.max],ps=ps,res=res,power.opt=power.opt,ysig=yp,par.opt=par.opt,lnBFs=lnBFs))
}
####extra routine
#####optimal parameters
##    Amin <- Bmin <- -2*(max(y)-min(y))
...
##    par.fix <- list(omega=omega.opt,phi=phi)
##    df <- list(t=t,y=y,dy=dy,par.fix=par.fix)
##    out <- nls.lm(par = start,lower=par.low,upper=par.up,fn =glst.res,df=df,control=nls.lm.control(maxiter=500))
##    par.full <- c(as.list(coef(out)),par.fix)
##    yp <- glst.model(t,par.full)
##    ll <- sum(dnorm(y,mean=yp,sd=err,log=T))
##    tmp <- lm(y~t, data=df)
##    yp0 <- tmp$coefficients[1]+tmp$coefficients[2]*t
##    ll0 <- sum(dnorm(y,mean=yp0,sd=err,log=T))
##    lnBF <- ll-ll0-1.5*log(N)
#####
...
glst.res <- function(par,df){
    ...
    phi <- df$par.fix$phi
    omega <- df$par.fix$omega
    (y-glst.model(t,c(par,df$par.fix)))/abs(dy)
}
