...
#        stop(length(index),'epochs are beyond the timespan of JPL ephemeris files and please get a ephemeris with longer timespan!\n')
        cat('Warning:',length(index),'epochs are beyond the timespan of JPL ephemeris files and the barycenter is used!\n')
        Nt <- nrow(tdb)
    }
    Nt <- nrow(tdb)
...
###use the barycenter as ephemerides for epochs earlier than JPL ephemerides
    ind <- which(is.na(out[,1]))
    if(length(ind)>0) out[ind,] <- 0
###barycenter of the Earth-moon center
    return(out)
}
