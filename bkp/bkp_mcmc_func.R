...
calc.astro <- function(K,P,e,inc,omega,Omega,E,plx,Mstar,pars,eta=NA,state=FALSE,band='G'){
    ...
    if(Np.kep>0){
        for(j in 1:Np.kep){
            if(pp$P[j]>Pmin){
                ...
                E <- kep.mt2(ms,pp$e[j])
                tmp <- calc.astro(pp$K[j],pp$P[j],pp$e[j],pp$Inc[j],pp$omega[j],pp$Omega[j],E,plx=plx,Mstar=Mstar,pars=pars.kep,band=band,eta=eta0[j])
...
###calculate BT
            if(pp$K[j]>0){
                bt <- calc.astro(pp$K[j],pp$P[j],pp$e[j],pp$Inc[j],pp$omega[j],pp$Omega[j],EP[[j]],plx=plx,Mstar=Mstar,state=TRUE,eta=eta0[j],pars=pars.kep)
                BT.all[[j]] <- bt
...
astrometry.hg <- function(pars.kep,tt=NULL,bases=rep('natural',10),Ein=NULL,pa=FALSE){
...
astrometry.kepler <- function(pars.kep,tt=NULL,bases=rep('natural',10),Pmin=1000){
    ...
    tmp <- list()
    if(length(out$astrometry)>0){
        tmp$barycenter <- astrometry.hg(pars.kep=pars.kep,tt=tt,bases=bases)
    }
    if(any(names(out)=='rel')){
...
cal.residual <- function(pars,bases=rep('natural',10),nqp=NULL){
    ...
    if(out$Nrv>0){
        for(k3 in 1:length(out[['ins']])){
            ...
            residual.sigtrend[[out[['ins']][k3]]] <- res.sigtrend
...
#            }else{
    ...
    if(any(e<0) | k==Ntt){
        cat('Proposing time exceeding the maximum 1000 times or negative eccentricity!!\n')
        stop()
    }
    return(param.new)
}
