...
ff <- '../paper/astrometry/v4/'

###paper BDs
#star <- c('HD 131664 B','GJ 777 B','GJ 779 B', 'HD 4747 B')
star <- c('HD 190360 b','HD 190406 B', 'HD 4747 B')
m <- c(1.762,72.83,68.26)
em <- c(0.24,6.1,5.9)
#a <- c(2.32,6.7,2,3.2)
T <- c(149.5,1680,1700)
eTl <- c(26.5,170,100)
...
fin <- paste0(ff,'cooling_age_mass.pdf')
cat(fin,'\n')
pdf(fin,6,6)
#par(mfrow=c(2,2))
size <- 1.5
...
plot(unlist(mass),unlist(teff),col='white',xlab=expression('Mass ['*M[Jup]*']'),ylab=expression(T[eff]*' [K]'),xlim=rev(c(1,80)),ylim=c(100,2000))
...
cols <- c('brown','black','green','orange')
ind2 <- ind <- 1:length(m)
...
arrows(m[ind2],T[ind2]-eTl[ind2],m[ind2],T[ind2]+eTu[ind2],length=0.1,angle=90,code=3,col=cols[ind2],cex=0.8,pch=20)
##add Jupiter
points(1,134,pch=15,col=cols[4])
legend('topright',legend=c(star,'Jupiter','ATMO 2020'),col=c(cols,'darkgrey'),pch=c(20,20,20,15,NA),lty=c(rep(NA,4),1),bty='n',cex=size)
dev.off()
...
fin <- paste0(ff,'cooling_age_mass_jupiter.pdf')
cat(fin,'\n')
pdf(fin,6,6)
#par(mfrow=c(2,2))
par(mar=c(5,5,1,1),cex.lab=size,cex.axis=size,cex.main=size)
