...
####for epochs without ephemerides we assume they are observed at the geocenter because the parallax is only modified by about 1% for satellites located at L2 and it does not infuence the target system roemer delay
    if(length(ind.na)>0) xyz[ind.na,] <- 0
...
####find UTC, TDB and the correct Earth's velocity interatively
    if(!sim){
        utc <- utc0 <- bjd <- cbind(floor(ts),ts%%1)
    }else{
    ...
    for(j in 1:Ntry){
        ...
        utc <- bjd+(OutObs$JDutc-OutObs$JDtdb)
        dt <- rowSums(utc-utc0)
