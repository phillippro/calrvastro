...
fs <- c('HD131664_fix1_relativityFALSE_Niter2000000_Ncores8_ofac2_Nset3_hg2--_transit0_P1980_acc6.3_sinI_lnlmax',
        'HD16160_fix1_relativityFALSE_Niter2000000_Ncores8_ofac2_Nset8_hg2--_transit0_P28220_acc2.9_sinI_lnlmax',
        'HD161797_fix1_relativityFALSE_Niter1500000_Ncores8_ofac2_Nset5_hg2--_transit0_P22293_acc1.9_sinI_lnlmax',
'HD190360_fix1_relativityFALSE_Niter1200000_Ncores8_ofac2_Nset6_hg2--_transit0_P2851_acc12_sinI_lnlmax',
'HD190406_fix1_relativityFALSE_Niter2000000_Ncores8_ofac2_Nset5_hg2--_transit0_P23487_acc7.1_sinI_lnlmax',
'HD39587_fix1_relativityFALSE_Niter1500000_Ncores8_ofac2_Nset5_hg2--_transit0_P5154_acc11_sinI_lnlmax',
'HD4747_fix1_relativityFALSE_Niter2000000_Ncores8_ofac2_Nset3_hg2--_transit0_P12091_acc6.9_sinI_lnlmax',
'HIP2552_fix1_relativityFALSE_Niter1500000_Ncores8_ofac2_Nset1_hg2--_transit0_P6011_acc0.18_sinI_lnlmax'
)
...
for(f in fs){
    ...
    if(length(ff)==0){
        ...
        p <- system(cmd)
        if(p==1){
            cmd <- paste0('scp -azP tdlffb@data.hpc..sjtu.edu.cn:Documents/projects/dwarfs/rvastro/results/',star,'/',f,'*Robj results/',star,'/')
            cat(cmd,'\n')
            system(cmd)
        }
    }
...
stars0 <- stars
####loading data
fobj <- paste0('hgca_phase',version,'.Robj')
#if(file.exists(fobj)){
if(FALSE){
    if(!exists('hgca_out.phase')){
        load(fobj)
    }
    reload <- FALSE
}else{
...
fout <- paste0('hgca_phase_coverage',version,'.pdf')
cat('output pdf:\n',fout,'\n')
pch.type <- 20
pch.size <- 1
Ncol <- 4
...
fout <- paste0('hgca_par',version,'.txt')
