...
#    ZgsO <- ZgO+ZsO
    ...
    if(Par$binary & Par$Np>0){
        if(Par$Einstein){
            RCT <- gen_CalLen(rTC)
            ZgT <- ZgT+rv_GravTarget(MC,RCT,g=1)
            if(!is.na(RT)){
                Zself <- rv_GravTarget(MT,RT*Rsun.au,g=1)#gravitational Doppler shift due to the target if the radius and mass of the target star is known
...
###consider non-GR scneario
#    cat('Par$gamma=',Par$gamma,'\n')
    ZgsO <- ZgsO*Par$gamma
    ZgsT <- ZgsT*Par$gamma
