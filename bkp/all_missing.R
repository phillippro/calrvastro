...
for(star in stars){
    ...
    ind.par <- grep(paste0(star,'_'),fpars)
    if(length(ind.par)>1) ind.par <- ind.par[which.max(as.numeric(gsub('.+_lnlmax|\\.par','',fpars[ind.par])))]
...
#        cat('p=',p,'\n')
        if(any(p>100,na.rm=TRUE)){
            if(file.exists(fpar)){
            pars <- read.table(fpar,header=TRUE)
            if(any(grepl('per',pars[,1]))){
