...
# Copy objects that the legacy MCMC helpers expect to find in the global
# environment (e.g. par.min / par.max).  The parallel workers execute the
# setup scripts inside the foreach frame, so we mirror the worker frame back
# into .GlobalEnv to reproduce the original serial behaviour and keep the
# functions sourced from mcmc_func.R working.
sync_mcmc_globals <- function(env, vars = NULL) {
    if (is.null(vars)) {
        # Mirror everything visible in the worker frame so that helper routines
        # sourced from mcmc_func.R observe the same globals as they would in the
        # serial execution path.  Keep the special handling for legacy bounds so
        # callers that depend on the previous behaviour still work when an
        # explicit var set is provided.
        vars <- ls(env, all.names = TRUE)
        vars <- setdiff(vars, c("sync_mcmc_globals", "prepare_parallel_worker"))
    } else {
        legacy <- union(vars, c("par.min", "par.max", "par.ini", "startvalue", "cov.start"))
        legacy <- union(legacy, ls(env, pattern = "\\.(min|max|ini)$", all.names = TRUE))
        vars <- unique(legacy)
    }

    for (nm in vars) {
        if (exists(nm, envir = env, inherits = FALSE)) {
            assign(nm, get(nm, envir = env, inherits = FALSE), envir = .GlobalEnv)
        }
    }
}
...
for (np in Nmin:Nmax) {
  ...
  # Build the foreach spec with backend-specific options *inside* foreach()
    fe <- foreach(
        ...
        .errorhandling = "pass",
        .inorder = FALSE,
        .export = c("prepare_parallel_worker", "sync_mcmc_globals", export_vars),
