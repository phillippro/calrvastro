...
fs <- c(
'HD131664_fix1_relativityFALSE_Niter1100000_Ncores16_ofac2_Nset3_hg3_transit0_P1982_acc0.66_sinI_lnlmax',
#'HD131664_fix1_relativityFALSE_Niter2300000_Ncores8_ofac2_Nset3_hg3_transit0_P1983_acc0.92_sinI_lnlmax',
'HD16160_fix1_relativityFALSE_Niter1200000_Ncores16_ofac2_Nset8_hg3_transit0_P27857_acc1.4_sinI_lnlmax',
'HD161797_fix1_relativityFALSE_Niter3200000_Ncores16_ofac2_Nset5_hg3_transit0_P26334_acc0.5_sinI_lnlmax',
'HD190360_fix1_relativityFALSE_Niter1000000_Ncores16_ofac2_Nset6_hg3_transit0_P2864_acc6_sinI_lnlmax',
'HD190406_fix1_relativityFALSE_Niter1000000_Ncores16_ofac2_Nset5_hg3_transit0_P22861_acc10_sinI_lnlmax',
'HD39587_fix1_relativityFALSE_Niter2100000_Ncores16_ofac2_Nset5_hg3_transit0_P5156_acc1_sinI_lnlmax',
'HD4747_fix1_relativityFALSE_Niter1200000_Ncores16_ofac2_Nset3_hg3_transit0_P12118_acc4.9_sinI_lnlmax',
'HIP2552_fix1_relativityFALSE_Niter1100000_Ncores16_ofac2_Nset1_hg3_transit0_P5688_acc0.08_sinI_lnlmax'
)
fs <- 'HD42581_fix1_relativityFALSE_Niter1300000_Ncores32_ofac2_Nset6_hg3\\+_transit0_P83046d16180_acc0.83_sinI_lnlmax'
...
for(f in fs){
    ...
    cat('input file:',ff,'\n')
    if(length(ff)==0){
        cmd <- paste0('scp ffeng@astro.tdli.sjtu.edu.cn:Documents/projects/dwarfs/rvastro/results/',star,'/',f,'*Robj results/',star,'/')
        cat(cmd,'\n')
        p <- system(cmd)
        if(p==1){
            cmd <- paste0('scp tdlffb@data.hpc.sjtu.edu.cn:Documents/projects/dwarfs/rvastro/results/',star,'/',f,'*Robj results/',star,'/')
            cat(cmd,'\n')
            system(cmd)
        }
    }
     f1 <- c(f1,list.files(paste0('results/',star),pattern=paste0(f,'.+Robj'),full.name=TRUE)[1])
}
fs <- f1
if(FALSE){
    if(version==1){
...
fout <- paste0('hg3_phase_coverage',version,'.pdf')
cat('output pdf:\n',fout,'\n')
pch.type <- 20
pch.size <- 1
Ncol <- 4
...
fout <- paste0('hg3_par',version,'.txt')
