...
###This is to give both absolute and relative astrometry if the absolute astrometry data is given
    ...
    if(any(names(pars.kep)=='dra0')) dra <- pars.kep['dra0']#as; note that this dra0 is ra
    if(any(names(pars.kep)=='ddec0')) ddec <- pars.kep['ddec0']
    if(any(names(pars.kep)=='dpmra0')) dpmra <- pars.kep['dpmra0']#mas/yr
    if(any(names(pars.kep)=='dpmdec0')) dpmdec <- pars.kep['dpmdec0']
...
#                tt <- c(as.numeric(out$astrometry[1,c('epoch_ra','epoch_dec')]),as.numeric(out$astrometry[2,c('epoch_ra','epoch_dec','ref_epoch')]))%%24e5-tmin%%24e5
...
RV.kepler <- function(pars.kep,tt=NA,prior.kep=prior.type,injection=FALSE,kep.only=FALSE,noise.only=FALSE,bases=rep('natural',10),rv.pm=0,nqp=NULL){
    ...
    if(Np.kep>0){
        ...
        if(out$Nrv>0){
            if(nepoch>0){
                if(!grepl('hgca',astro.type)){
                    tt1 <- c(out$astrometry[,1]-tmin,tt)
                }else{
                    tt1 <- c(c(as.numeric(out$astrometry[1,c('epoch_ra','epoch_dec')]),as.numeric(out$astrometry[2,c('epoch_ra','epoch_dec')]))-tmin,tt)
                }
            }
###add relativistic effects
            ...
            if(out$relativity){
                ...
                if(nepoch>0){
                    ind.rv <- (nepoch+1):length(tt1)
                    for(kk in 1:Np.kep){
                        Eastro[[kk]] <- tmp$Ein[[kk]][1:nepoch]
...
#likelihood
loglikelihood <- function(pars,bases=rep('natural',10),RVonly=FALSE,nqp=NULL,prediction=FALSE,indrel=NULL){
...
#            if(!is.null(logLike))cat('logLike=',logLike,'\n')
#            cat('ll=',ll,'\n')
        }
    }
