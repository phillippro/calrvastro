...
if(astrotype=='hgca3'){
   name.alt <- name.alt[1:7]
}
fs <- paste0('results/',gsub('pdf','Robj',read.table(paste0(astrotype,'.txt'))[,1]))
...
stars0 <- stars
####loading data
fobj <- paste0(astrotype,'_phase',version,'.Robj')
#if(file.exists(fobj)){
if(FALSE){
    if(!exists(paste0(astrotype,'_out.phase'))){
        load(fobj)
    }
    reload <- FALSE
}else{
...
nc <- length(sets)
colors <- c(brewer.pal(9, "Set1"),brewer.pal(n = length(stars), name = "Set2"))
#colors <- rainbow(nc)
...
fout <- paste0(astrotype,'_phase_coverage',version,'.pdf')
cat('output pdf:\n',fout,'\n')
pch.type <- 20
pch.size <- 1
Ncol <- 4
...
ind1 <- 1:(length(stars)-1)
ind2 <- length(stars):length(ins.all)
...
fout <- paste0(astrotype,'_par',version,'.txt')
