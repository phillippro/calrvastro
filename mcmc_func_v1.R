...
####relative astrometry reference: https://arxiv.org/pdf/2109.10671.pdf
            if(!is.null(tt)){
                ...
                ms <- (pp$Mo[j]+2*pi*(out1$tau[out1$ind.rel]-out1$tref)/pp$P[j])%%(2*pi)
            }
            E <- kep.mt2(ms,pp$e[j])
            plx <- out1$plx
...
#t already correct for solar system roemer delay
        tau <- t-roemer#include both tauC and tauT and will be separated after the iteration
        EP <- BT.all <- list()
        BT <- 0
        roemer <- 0
        for(j in 1:Np.kep){
###calculate BT
            if(pp$K[j]>0){
                ...
                BT.all[[j]] <- bt
                BT <- BT+bt[indP,]
                roemer <- roemer+rowSums(bt[,1:3]*out1$uOB)/Cauday#approximation: actually bt be projected onto uOT or uOC to get relevant delay
            }else{
                BT.all[[j]] <- 0
            }
###decide whether to repeat
        ...
        if(binarity){
            roemer[indC] <- -roemer[indC]*Mstar/Mps[imax]
            BT[indC] <- -BT[indC]*Mstar/Mps[imax]
        }
        if(max(abs(roemer-roemer0))<tol) break()
        roemer0 <- roemer
    }
#    if(binarity){
#        BC <- -BT.all[[imax]][indC,]*
#        roemerC <- rowSums(BC[,1:3]*out1$uOB[indC,])/Cauday#approximation because uOB should be uOT
#        tauC <- t[indC]-roemerC
#        tauT <- tauT[indP]
#        BT <- BT[indP,]
#    }
    ...
    for(j in 1:Np.kep){
        if(pp$K[j]>0){
            ...
            RCT[[j]] <- sqrt(rowSums((BT.all[[j]][,1:3]/eta)^2))
            zg <- zg+SRS*Mps[j]*(1+gamma)/4*(1/RCT[[j]][indP]-1/as[j]))#relative RV
        }else{
            RCT[[j]] <- list(NULL)
        }
    }
    if(binarity) zg[indC] <- zg[indC]*Mstar/Mps[imax]
...
###add reflex motion
        uOB <- out1$uOB+du
        duP <- du[indP,]+BT[,1:3]/pc2au/(1000/ast2[indP,'parallax'])#change of direction in rad
#        uOT <- gen_CalUnit(out1$uOB+duP)
        uOT <- gen_CalUnit(-out1$rSO/pc2km+ast2[,c('x','y','z')]+BT[,1:3]/pc2au)

###add reflex motion to binary companion
        drvT.du <- rowSums(out1$vSO[indP,]*duP*1e3)#m/s
        rvSOt <- rowSums(out1$vSO[indP,]*uOT*1e3)#m/s
...
#    rvST <- ast2[,'radial_velocity']*1e3+rowSums(BT[,4:6]*uOT)*4.74047e3#m/s;
    ...
    rv.sys <- rowSums(ast2[indP,c('vx','vy','vz')]*uOT)*auyr2kms*1e3
    rvST <- rv.sys+rv.kep
#    ZlocalT <- (1-out1$ZgsO)/(1+rvSOt/CMPS-out1$ZlO)-1
#stop()
    ZlocalT <- (1-out1$ZgsO[indP])/(1+rvSOt/CMPS)-1
    ...
    Zm <- (ZlocalT+1)*(1+rvST/CMPS)/(1+rv0/CKMPS)-1#eq. 24 of Wright & Eastman 2014
    rvST0 <- obs['radial_velocity']*1e3+rowSums(BT[,4:6]*out1$uOB[indP,])*4.74047e3
    ZTw <- (1+rvSOt/CMPS)/(1-out1$ZgsO[indP])*(1+rvST0/CMPS)/(1+rvST/CMPS)-1
#    ZTw <- 1/(ZT+1)-1

    if(binarity){
        rvc.kep <- rowSums(BC[,4:6]*uOC)*auyr2kms*1e3#m/s
        rvc.sys <- rowSums(ast2[indC,c('vx','vy','vz')]*uOC)*auyr2kms*1e3
        rvSC <- rvc.sys+rvc.kep
        ZlocalC <- (1-out1$ZgsO[indC])/(1+rvSOc/CMPS)-1
        ZremoteC <- (1+rvST/CMPS)/(1-rvgsT/CMPS)-1#ignore ZlT
        rvc0 <- rep(0,length(tauC))
        if(!sim.kep){
            for(ins in out1$ins.binary){
                ind <- out1$ind.binary[[ins]]
                rvc0[ind] <- pars.kep[paste0('rv0b_',ins)]
            }
        }
        ZmC <- (ZlocalC+1)*(1+rvSC/CMPS)/(1+rvc0/CKMPS)-1#eq. 24 of Wright & Eastman 2014
    }
...
if(FALSE){
    ...
    return(list(rvkep=rv.kep,rvckep=rvc.kep,rvsys=rv.sys,rvgsT=rvgsT,rvgT=rvgT,rvsT=rvsT,tau=tau,tauT=tauT,tauC=tauC,roemerT=roemerT,drvT.du=drvT.du,EP=EP,roemerC=roemerC,drvC.du=drvC.du,ZT=ZT,Zm=Zm,ZTw=ZTw,ZlocalT=ZlocalT))
}
...
#            if(!sim) tt <- out1$data.epoch[[i]][,1]#JD or BJD
            if(!sim) tt <- out1$tauT[out1$ind.all[[paste0('epoch',1)]][[i]]]
            if(grepl('hip',i)){
                band <- 'Hp'
            }else{
                band <- 'G'
...
####calculate the dra and ddec relative to the Gaia DR3 reference epoch
            if(!grepl('hip|TYC',i)){
                if(!sim){
                    plx.ra <- out1$pf[out1$ind.all$epoch[[i]],'ra']
                    plx.dec <- out1$pf[out1$ind.all$epoch[[i]],'dec']
                    bary <- tmp$barycenter[out1$ind.all$epoch[[i]],]
                }else{
                    plx.ra <- out1$pf[,'ra']
                    plx.dec <- out1$pf[,'dec']
                    bary <- tmp$barycenter
...
RV.kepler <- function(pars.kep,tt=NULL,out1=out,prior.kep=prior.type,injection=FALSE,kep.only=FALSE,noise.only=FALSE,bases=rep('natural',10),rv.pm=0,nqp=NULL){
    ...
    if(length(out1$data.binary)>0) binarity <- TRUE
    if(!sim.kep){
        if(any(names(out)=='BJDtcb') & dtype=='raw'){
            tt <- out1$BJDtcb
        }else{
            tt <- out1$tiall[,1]
        }
...
#        if(out1$Nrv>0){
        if(length(tt)>0){
            if(dtype=='raw' & out1$Nrv>0){
                tmp <- RV.relativity(pars.kep,out1=out1,t=tau,indP=indP,indC=indC,Mstar=Mstar,Mps=Mps,as=as,pp=pp,sim.kep=sim.kep)
                if(nepoch>0){
                    indP <- 1:length(tt)
                    for(kk in 1:Np.kep){
...
#                drvT <- -tmp$ZTw*CMPS#-out1$rvs
                ...
                drvT <- tmp$Zm*CMPS#-out1$rvs
#                cat('head(drvT)=',head(drvT[out$ind.all$rv$PFS]),'m/s\n')
#                cat('head(out$PFS$RV[,2])=',head(out$PFS$RV[,2]),'m/s\n')
...
#                    rvC <- rvC+tmp$rvgsC+tmp$drvC.du
                ...
                for(h in 1:Np.kep){
                    ...
                    if(binarity){
                        EC <- (pp$Mo[imax]+2*pi*(tauC-out1$tref)/pp$P[h])%%(2*pi)
...
###moon mass
        ...
        drvbary <- drvkep <- drvT <- rep(0,length(tt))
        if(binarity) drvC <- rep(0,nrow(out1$data.binary))
    }
...
###calculate noise component
    RVbary <- RVkep <- RVT <- RVC <- RVkepC <- list()
    if(sim.kep){
        Nins <- 1
    }else{
...
###assign parameters and data
            ...
            if(!sim.kep){
#                dVr.kep[[out1$ins.rv[j2]]] <- rvP[out[[out1$ins.rv[j2]]]$index]
                RVT[[ins]] <- drvT[ii]
                RVkep[[ins]] <- drvkep[ii]
                RVbary[[ins]] <- rep(0,length(ii))
            }else{
                RVT <- drvT
                RVkep <- drvkep
                RVbary <- drvbary
            ...
            if(!kep.only){
                ...
                if(!sim.kep){
                    trend <- cal.trend(a=at,b=b,t=(tt[ii]-out1$tref)/time.unit)
                    RVT[[out1$ins.rv[j2]]] <- RVT[[out1$ins.rv[j2]]]+trend
                }else{
...
###check whether PM-induced RV trend is subtracted; perspective acceleration
                if(grepl('VLC|LC|CFHT',out1$ins.rv[j2]) & length(out1$astrometry)>0){
                    ...
                    RVT[[out1$ins.rv[j2]]] <- RVT[[out1$ins.rv[j2]]]+rv.pm
                }
                ...
                if(nqp[1]>0){
                    ...
                    if(length(ind)>0){
                        ...
                        RVT[[out1$ins.rv[j2]]] <- RVT[[out1$ins.rv[j2]]] + tmp
                    }
                }
            }
        }
...
#####binary companion RV
    if(binarity & offset & !sim.kep){
        ...
        for(j in 1:Nins){
            ...
            if(offset) b <- pars.kep[paste0('bb_',ins)]
            RVC[out1$ind.binary[[ins]]] <- RVC[out1$ind.binary[[ins]]]+b
        }
    }
    return(list(tt=tt,E=Ein,rv=RVT,rvkep=RVkep,rvC=RVC,rvkepC=RVkepC,rvbary=RVbary,rvg=rvgP,drvT=drvT,drvT.du=drvT.du,rvs=rvsP,rvsC=rvsC,rvgC=rvgC,drvC=drvC,tauC=tauC,rvc=rvc,rvm=rvm,tau=tau,tauT=tauT))
}
...
#par(mfrow=c(4,4))
    ...
    if(out1$Nrv>0){
        for(k3 in 1:length(ins)){
            ...
            rv.model <- rv.kep <- RV.kep[[out1$ins.rv[k3]]]
            if(is.null(nqp.all)){
                nqp <- out[[out1$ins.rv[k3]]]$noise$nqp
            }else{
...
#           res <- out1$data.binary[out1$ind.binary[[ins]],2]-rvp$drvC.du[out1$ind.binary[[ins]]]
           res <- out1$data.binary[out1$ind.binary[[ins]],2]-rvp$rvC[out1$ind.binary[[ins]]]
           jitter <- pars[paste0('Jb_',ins)]
           ll <- sum(dnorm(res, mean=0, sd=sqrt(out1$data.binary[out1$ind.binary[[ins]],3]^2+jitter^2), log=TRUE))
           logLike <- logLike+ll
...
cal.residual <- function(pars,out1=out,bases=rep('natural',10),nqp=NULL){
    ...
    if(out1$Nrv>0){
        for(k3 in 1:length(out1$ins.rv)){
            trv <-out[[out1$ins.rv[k3]]]$RV[,1]
            rv.data <- out[[out1$ins.rv[k3]]]$RV[,2]
            erv <- out[[out1$ins.rv[k3]]]$RV[,3]
            rv.all <- RV.all[[out1$ins.rv[k3]]]
            rv.sig <- RV.sig[[out1$ins.rv[k3]]]
            rv.kep <- RV.kep[[out1$ins.rv[k3]]]
            ...
            if((nqp[2]>0 | nqp[3]>0)){
                rv.arma <- arma(t=trv,ymodel=rv.all,ydata=rv.data,pars=pars,set=out1$ins.rv[k3],p=nqp[3],q=nqp[2])
                if(FALSE){
...
#stop()
            ...
            residual.all[[out1$ins.rv[k3]]] <- res.all
            residual.sig[[out1$ins.rv[k3]]] <- res.sig
            residual.sigtrend[[out1$ins.rv[k3]]] <- res.sigtrend
            residual.noise[[out1$ins.rv[k3]]] <- res.noise
            signal[[out1$ins.rv[k3]]] <- rv.sig
            noise[[out1$ins.rv[k3]]] <- rv.noise
            all[[out1$ins.rv[k3]]] <- rv.all
            arma1[[out1$ins.rv[k3]]] <- rv.arma
            trend[[out1$ins.rv[k3]]] <- rv.trend
        }
    }
    list(res.kep=res.kep,res.sig=residual.sig,res.sigtrend=residual.sigtrend,res.all=residual.all,res.noise=residual.noise,rv.bary=RV.bary,rv.sig=RV.sig,rv.kep=RV.kep,rv.signal=signal,rv.noise=noise,rv.all=all,rv.arma=arma1,rv.trend=trend)
}
