...
#fs <- fs[grep('HD198387',fs):length(fs)]
#fs <- fs[grep('HD198387',fs):length(fs)]
#fs <- paste0('results/',gsub('pdf','Robj',read.table(paste0(comptype,'_files_sc.txt'))[,1]))
#fs <- paste0('results/',gsub('pdf','Robj',read.table(paste0(comptype,'_files_ne.txt'))[,1]))
Nf <- length(fs)
cat('Number of files:',Nf,'\n')
...
ind.out <- c()
par.all <- foreach(k5 = 1:length(fs), .combine='c', .multicombine=TRUE) %dopar% {
snr <- Trv <- Nrv <- RVrms <- mstars <- emstars <- par.out <- c()
#for(k5 in 1:length(fs)){
    ff <- fs[k5]
     astro.type <- gsub('.+Nset\\d_|_transit.+','',ff)
#    cat('ff:\n',ff,'\n')
    target0 <- targets0[k5]
...
#    nastro <- e0$out$Nastro
  ...
  nn[nn=='ddec0'] <- 'ddec'
  nn[nn=='dpmra0'] <- 'dpmra'
  nn[nn=='dpmdec0'] <- 'dpmdec'
  nn[nn=='dplx0'] <- 'dplx'
  nn <- gsub('logJ.','logJ_',nn)
  nn <- gsub('hip$','hip2',nn)
  names(e0$par.opt) <- nn
  if(!any(names(out)=='iref')) out$iref <- 1
  tmp <- astrometry.kepler(e0$par.opt,bases=bases)
    astro0 <- tmp$barycenter
    rel <- tmp$rel
    tsim0 <- seq(data.astrometry[1,1],data.astrometry[2,1],length.out=1e4)
    tsim <- tsim0-tmin
    Nt <- length(tsim)
#    planet <- astrometry.kepler(e0$par.opt,tt=tsim,bases=bases)$planet
    ...
    cov.astro <- e0$cov.astro
    snr.pos <- sqrt(diff(dra)^2+diff(ddec)^2)/sqrt(sum(data.astrometry[,'ra_error']^2+data.astrometry[,'dec_error']^2))
    snr.pm <- sqrt(diff(dpmra)^2+diff(dpmdec)^2)/sqrt(sum(data.astrometry[,'pmra_error']^2+data.astrometry[,'pmdec_error']^2))
#snr <- c(snr,sqrt(snr.pos^2+snr.pm^2))
    snr <- sqrt(snr.pos^2+snr.pm^2)
    cat('snr=',snr,'\n')
#	source('snr.R',local=TRUE)
#	source('snr.R')
...
###Nrv and RVrms
    rv.all <- out$all
    inds <- which(rv.all[,3]<10)
    if(length(inds)>0){
    RVrms <- c(RVrms,sd(out$res.comb.sig[[paste0('sig',Nsig)]][inds]))
    Nrv <- c(Nrv,nrow(out$all[inds,]))
    Trv <- c(Trv,max(out$all[inds,1])-min(out$all[inds,1]))
}else{
    RVrms <- c(RVrms,sd(out$res.comb.sig[[paste0('sig',Nsig)]]))
    Nrv <- c(Nrv,nrow(out$all))
    Trv <- c(Trv,max(out$all[,1])-min(out$all[,1]))
}
    ins.name <- ins
    if(any(ins=='HARPS')){
        th <- rv.list$HARPS[,1]
...
inss <- unique(inss)
#colnames(par.out) <- c('Star','P','Plow','Pup','K','Klow','Kup','e','elow','eup','omega','omega.low','omega.up','Omega','Omega.low','Omega.up','mp','mp.lower','mp.upper','snr')
par.out <- cbind(target0,mstars,emstars,Nrv,RVrms,Trv,par.out,snr)
#par.out <- cbind(targets0,mstars,emstars,par.out)
...
#fout <- paste0(comptype,'_par',version,'_',opt,'_ne.txt')
cat(fout,'\n')
par.out <- par.out[-1,]
