...
if(length(out$data.epoch)>0){
    ...
    for(j3 in 1:length(out$ins.epoch)){
        i  <- out$ins.epoch[j3]
        reflex.sim <- astrometry.epoch(par.opt,tt=tsim,comp=out$comp.epoch[j3])$epoch
...
###The following approach may not be good to show the residual relatied to long period companion because the stellar reflex motion is subtraced from the Gaia epoch and then propagate the Gaia epoch to Hipparcos epoch which will result in huge offset if without reflex motion
        ...
        if(i=='hip1'){
            ...
            dra.res <- pred.res$res[[paste0('epoch_',i)]][1:nepoch]
            ddec.res <- pred.res$res[[paste0('epoch_',i)]][nepoch+(1:nepoch)]
...
#            k1 <- 1:nrow(out$astrometry)
            ...
            for(i1 in k1){
                ell <- error.ellipse(dra.ref[i1],ddec.ref[i1],out$cov.astro[1:2,1:2,i1],percent=68)
...
#out$astrometry[imax,'pmdec']*dt.sim+out$astrometry[out$iref,'parallax']*out$pf.sim[,'dec']+par.opt[paste0('ddec_',i)]
#out$astrometry[imax,'pmra']*dt.sim+out$astrometry[out$iref,'parallax']*out$pf.sim[,'ra']+par.opt[paste0('dra_',i)]
...
###parallax projected onto R.A. and Decl.
                pfra.sim <- plx.opt*out$pf.sim[,1]
                pfdec.sim <- plx.opt*out$pf.sim[,2]
                ...
                plot(tsim,pfra.sim,xlab='BJD',ylab=expression(Delta*alpha*'* [mas]'),xlim=range(tobs),ylim=range(pfra.obs,pfra.sim),main=paste(i,'parallax'),type='l',col='red')
                points(tobs,pfra.obs)
                arrows(tobs,pfra.obs-era,tobs,pfra.obs+era,length=0.001,angle=0,code=1,col='grey')
                plot(tsim,pfdec.sim,xlab='BJD',ylab=expression(Delta*delta*' [mas]'),xlim=range(tobs),ylim=range(pfdec.obs,pfdec.sim),main=paste(i,'parallax'),type='l',col='red')
...
##combined
    ...
    index <- which(tsim>=(min(out$astrometry[,'ref_epoch'])-100) & tsim<=(max(epochs)+100))
    dtsim <- (tsim[index]-out$astrometry[out$iref,'ref_epoch'])/365.25#yr
    pfra.sim <- out$pf.sim[index,'ra']
    pfdec.sim <- out$pf.sim[index,'dec']
    bary.sim <- astrometry.bary(par.opt,bases=bases,tt=tsim)[index,]
    rd <- obs.lin.prop(out$astrometry[out$iref,],tsim[index]-out$astrometry[out$iref,'ref_epoch'],PA=TRUE)
    dras <- (rd[,1]-out$astrometry[out$iref,'ra'])*cos(out$astrometry[out$iref,'dec']/180*pi)*3.6e6+out$astrometry[out$iref,'parallax']*out$pf.sim[index,'ra']
    ddecs <- (rd[,2]-out$astrometry[out$iref,'dec'])*3.6e6+out$astrometry[out$iref,'parallax']*out$pf.sim[index,'dec']
    ...
    dra.all.sim <- dra.linear.sim+reflex.sim$dra[index]
    ddec.all.sim <- ddec.linear.sim+reflex.sim$ddec[index]
...
###the position and proper motion reconstructed by a linear fit to gost-based observations generated by accounting for reflex motion
        ...
        reflex.gost.binary <- -astrometry.epoch(par.opt1,tt=out[[binary]]$gost[,'BJD'],bases=bases)$epoch/mratio
        index.binary <- which(tsim>=(min(out[[binary]]$astrometry[,'ref_epoch'])-100) & tsim<=(max(epochs)+100))
        rd.binary <- obs.lin.prop(out[[binary]]$astrometry[out[[binary]]$iref,],tsim[index.binary]-out[[binary]]$astrometry[out[[binary]]$iref,'ref_epoch'],PA=TRUE)
        dras.binary <- (rd.binary[,1]-out[[binary]]$astrometry[out[[binary]]$iref,'ra'])*cos(out[[binary]]$astrometry[out[[binary]]$iref,'dec']/180*pi)*3.6e6+out[[binary]]$astrometry[out[[binary]]$iref,'parallax']*out$pf.sim[index.binary,'ra']
        ddecs.binary <- (rd.binary[,2]-out[[binary]]$astrometry[out[[binary]]$iref,'dec'])*3.6e6+out[[binary]]$astrometry[out[[binary]]$iref,'parallax']*out$pf.sim[index.binary,'dec']
