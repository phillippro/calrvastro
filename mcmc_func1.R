...
plot.labels.simple <- function(par.name){
    ...
    pat.noise <- c('a','b','beta','alpha','w','m','tau','s','c','logJ','jitter','dra','ddec','dpmra','dpmdec','dplx','drv')
...
calc.astro <- function(K,P,e,inc,omega,Omega,E,Mstar,eta=1,state=FALSE,Plps=1000,Ts=1000){
...
#    beta <- (P*3600*24)*K*sqrt(1-e^2)/(2*pi)*plx/sin(inc)*6.68459e-12#mas
    raP.epoch <- beta*(B*X+G*Y)
    decP.epoch <- beta*(A*X+F*Y)
    pmraP.epoch <- alpha*(B*Vx+G*Vy)
    pmdecP.epoch <- alpha*(A*Vx+F*Vy)
...
####calculate PM
	...
	raP <- raP.epoch
	decP <- decP.epoch
    }else if(P<min(Ts) & P>min(Ts)/2 & shortP){
####for periods slightly shorter than the baseline Ts, the modulo of the baseline should be used (though not accurate)
        pmraP <- -pmraP.epoch*(Ts%%P)/Ts
	pmdecP <- -pmdecP.epoch*(Ts%%P)/Ts
	raP <- -raP.epoch
	decP <- -decP.epoch
    }else{
	pmraP <- rep(0,length(T))
	pmdecP <-rep(0,length(T))
	raP <-rep(0,length(T))
	decP <-rep(0,length(T))
    }
    ...
    if(eta!=1){
        mp <- K2msini.full(K,P,e,Mstar)$ms/sin(inc)#in unit of solar mass
        if(mp>0.01*Mstar){
                                        #        cat('Mstar/mp=',Mstar/mp,'\n')
                                        #        cat('mp/Mstar=',mp/Mstar,'\n')
    ...
    if(state){
        C <- sin(omega)*sin(inc)
        H <- cos(omega)*sin(inc)
        BT <- cbind(raP/plx,decP/plx,beta0*(C*X+H*Y),pmraP/plx,pmdecP/plx,alpha0*(C*Vx+H*Vy))#au,au/yr
        return(BT)
    }else{
...
astrometry.rel  <- function(pars.kep,Ein=NULL,tt=NULL,bases=rep('natural',10),pp=NULL,tsim=NULL,eta=1){
...
####add inner planet mass to mass center
            ...
            if(!is.null(tt) | !is.null(tsim)){
                ...
                tmp <- calc.astro(pp$K[j],pp$P[j],pp$e[j],pp$Inc[j],pp$omega[j],pp$Omega[j],E,Mstar=,eta=etaH)
...
##relative astrometry reference: https://arxiv.org/pdf/2109.10671.pdf
            if(any(names(out)=='rel') | !is.null(tsim)){
                if(out$Nrel[Np.kep]>0 | !is.null(tsim)){
                    if(!is.null(tsim)){
                    ...
                    E <- kep.mt2(ms,pp$e[j])
                    tmp <- calc.astro(pp$K[j],pp$P[j],pp$e[j],pp$Inc[j],pp$omega[j],pp$Omega[j],E,Mstar=mm,eta=etaH)
#                    tmp <- calc.astro(pp$K[j],pp$P[j],pp$e[j],pp$Inc[j],pp$omega[j],pp$Omega[j],E,Mstar=pars.kep['Mstar'],eta=1)
                    ...
                    xi <- Mp/(mm+Mp)
                    pmraR <- cbind(pmraR,tmp[,1])#reflex motion
                    pmdecR <- cbind(pmdecR,tmp[,2])
                    ...
                    if(!is.null(tsim)){
                        ...
                        for(i in inss){
                            raB[[n]][[i]] <- -tmp[out$indrel[[n]][[i]],3]/xi
                            decB[[n]][[i]] <- -tmp[out$indrel[[n]][[i]],4]/xi
                            pmraB[[n]][[i]] <- -tmp[out$indrel[[n]][[i]],1]/xi
                            pmdecB[[n]][[i]] <- -tmp[out$indrel[[n]][[i]],2]/xi
                            if(kk>1){
        ...
        if(FALSE){
####correct the astrometric perturbation from other planets by adding all other reflex motions to the star
                    ...
                    for(j in 1:Np.kep){
                        ...
                        if(length(out$indrel[[j]])>0 & ncol(raR)>1){
                            if(!is.null(tsim)){
                                ...
                                pmraB[[n]] <- pmraB[[j]]+rowSums(pmraR[,-j,drop=FALSE])
                                pmdecB[[n]] <- pmdecB[[j]]+rowSums(pmdecR[,-j,drop=FALSE])
                            }else{
                                raB[[n]][[i]] <- raB[[n]][[i]]+rowSums(raR[out$indrel[[n]][[i]],-j,drop=FALSE])
                                decB[[n]][[i]] <- decB[[n]][[i]]+rowSums(decR[out$indrel[[n]][[i]],-j,drop=FALSE])
                                pmraB[[n]][[i]] <- pmraB[[n]][[i]]+rowSums(pmraR[out$indrel[[n]][[i]],-j,drop=FALSE])
                                pmdecB[[n]][[i]] <- pmdecB[[n]][[i]]+rowSums(pmdecR[out$indrel[[n]][[i]],-j,drop=FALSE])
                            }
                        }
                    }
                }
    ...
    return(list(ra=ra.planet,dec=dec.planet,pmra=pmra.planet,pmdec=pmdec.planet,rel=list(raB=raB,decB=decB,pmraB=pmraB,pmdecB=pmdecB)))
}
...
#    save(list=ls(all=TRUE),file='test3.Robj')
    for(j in 1:Np.kep){
        m <- (pp$Mo[j]+2*pi*t[indP]/pp$P[j])%%(2*pi)
        EP[[j]] <- kep.mt2(m,pp$e[j])%%(2*pi)
    }
    if(binarity){
        m <- (pp$Mo[1]+2*pi*t[indC]/pp$P[1])%%(2*pi)
        EC <- kep.mt2(m,pp$e[1])%%(2*pi)
    }
...
###calculate BT
            if(pp$K[j]>0){
                bt <- calc.astro(pp$K[j],pp$P[j],pp$e[j],pp$Inc[j],pp$omega[j],pp$Omega[j],EP[[j]],Mstar=Mstar,state=TRUE)
                BT.all[[j]] <- bt
...
###decide whether to repeat
        ...
        if(binarity){
            bt <- calc.astro(pp$K[1],pp$P[1],pp$e[1],pp$Inc[1],pp$omega[1],pp$Omega[1],EC,Mstar=Mstar,state=TRUE)
            BC <- -bt*Mstar/Mps[1]
            roemerC <- BC[,3]/Cauday
        }
...
####Roemer delay
    tau[indP] <- t[indP]-roemerT#barycentric time to emission time; without relativitistic correction
    tau[indC] <- t[indC]-roemerC#barycentric time to emission time; without relativitistic correction

####eccentrici anomalty
    BT <- 0
    for(j in 1:Np.kep){
            m <- (pp$Mo[j]+2*pi*tau[indP]/pp$P[j])%%(2*pi)
            EP[[j]] <- kep.mt2(m,pp$e[j])%%(2*pi)
            if(!is.null(BT.all)) BT <- BT+BT.all[[j]]
    }
    if(binarity){
        m <- (pp$Mo[1]+2*pi*tau[indC]/pp$P[1])%%(2*pi)#assume that the first companion is identified as stellar binary component
        EC <- kep.mt2(m,pp$e[1])%%(2*pi)
    }
...
#    VBT <- rowSums(BT[,4:6]^2)#au/yr
    if(length(out$astrometry)==0){
        ...
        i <- which.max(out$astrometry[,'ref_epoch'])
        vBx <- out$astrometry[i,'pmra']/out$plx#au/yr
        vBy <- out$astrometry[i,'pmdec']/out$plx#au/yr
        vBz <- out$astrometry[i,'radial_velocity']/auyr2kms#au/yr
    }
...
###relative relativistc RV for primary
    ...
    rvgT <- gamma*rowSums(zgP)*CMPS#m/s
    rvgsT <- rvgT+rvsT
###relative relativistc RV for binary companion
    if(binarity){
        tau[indC] <- t[indC]-roemerC
        TC <- -bt*(Mstar+Mps[1])/Mps[1]
        RTC <- sqrt(rowSums(TC^2))
        zgC <- SRS*Mstar*(1+gamma)/4*(1/RTC-1/as[1])#GR relativistic
        rvgC <- gamma*zgC*CMPS#m/s
...
###calibrate the radial velocity by accounting for astrometric bias
    ...
    if(any(grepl('dra$',names(pars.kep))) & length(out$astrometry)>0){
        i <- which.max(out$astrometry[,'ref_epoch'])
        obs0 <- unlist(out$astrometry[i,c('ra','dec','parallax','pmra','pmdec','radial_velocity')])#reference astrometry
        obs <- obs0-c(pars.kep[c('dra','ddec')]/3.6e6,pars.kep[c('dplx','dpmra','dpmdec','drv')])#corrected astrometry
        ast1 <- obs.lin.prop(obs0,tau-(Par$tpos-tmin))
        ast2 <- obs.lin.prop(obs,tau-(Par$tpos-tmin))
        dast <- ast2-ast1
        du <- t(outer(out$du[,1],dast[,1]/180*pi,'*'))+t(outer(out$du[,2],dast[,2]/180*pi,'*'))
###add reflex motion
        duP <- du[indP,]+BT[,1:3]/pc2au/(1000/out$astrometry[i,'parallax'])#change of direction in rad
###add reflex motion to binary companion
        if(binarity){
            duC <- du[indC,]+BC[,1:3]/pc2au/(1000/out$astrometry[i,'parallax'])#change of direction in rad
        }else{
            duC <- 0
        }
        if(!sim.kep){
            drvT <- rowSums(out$vSO[indP,]*duP[indP,]*1e3)#m/s
            if(binarity) drvC <- rowSums(out$vSO[indC,]*duC*1e3)#m/s
        }else{
            drvT <- rowSums(out$vSO.sim*duP[indP,]*1e3)
            if(binarity) drvC <- rowSums(out$vSO.sim*duC*1e3)
        }
    }
    return(list(rvgsT=rvgsT,rvgT=rvgT,rvsT=rvsT,tau=tau,roemerT=roemerT,drvT=drvT,rvgsC=rvgsC,rvgC=rvgC,rvsC=rvsC,EP=EP,EC=EC,roemerC=roemerC,drvC=drvC))
}
...
###This is to give both absolute and relative astrometry if the absolute astrometry data is given
    ...
    Ks <- pp$K
    Mos <- pp$Mo
    dra <- ddec <- dpmra <- dpmdec <- dplx <- drv <- 0
    if(any(names(pars.kep)=='dra')) dra <- pars.kep['dra']#as; note that this dra0 is ra
    if(any(names(pars.kep)=='ddec')) ddec <- pars.kep['ddec']
    if(any(names(pars.kep)=='dpmra')) dpmra <- pars.kep['dpmra']#mas/yr
    if(any(names(pars.kep)=='dpmdec')) dpmdec <- pars.kep['dpmdec']
    if(any(names(pars.kep)=='dplx')) dplx <- pars.kep['dplx']
    if(any(names(pars.kep)=='drv')) drv <- pars.kep['drv']
    if(any(names(out)=='rel')){
        Mstar <- pars.kep['Mstar']
    }else{
        Mstar <- out$Mstar
...
###model parallax and mean proper motion; only assumption: constant heliocentric velocity; reference Gaia epoch
    ...
    if(grepl('hgca',astro.type)) ind.ref <- 3:4
    tt0 <- proc.time()
    if(Np.kep>0){
        for(j in 1:Np.kep){
...
###check whether PM-induced RV trend is subtracted
    ...
    if(any(grepl('VLC|LC|CFHT',ins)) & pa & out$Nrv>0){
        ...
        for(j4 in ind){
            t3 <- out[[out$ins[j4]]]$RV[,1]%%24e5-max(data.astrometry[,1])%%2400000
            tmp <- obs.lin.prop(obs,t3)
            rv.trend <- (tmp[,'radial_velocity']-tmp[1,'radial_velocity'])*1e3#m/s
            rv.pm[[out$ins[j4]]] <- rv.trend
        }
    }
...
###model planet-induced ra dec and PM variation
    ...
    if(!grepl('hgca',astro.type)){
        ...
        pmra <- c(obs1[,'pmra'],rep(obs['pmra'],2))+planet$pmra
        pmdec <- c(obs1[,'pmdec'],rep(obs['pmdec'],2))+planet$pmdec
        ra <- c(obs1[,'ra'],rep(obs['ra'],2))+planet$ra/3.6e6/cos(obs1[,'dec']/180*pi)
        dec <- c(obs1[,'dec'],rep(obs['dec'],2))+planet$dec/3.6e6
        plx <- c(obs1[,'parallax'],rep(obs['parallax'],2))
        astro <- cbind(ra,dec,plx,pmra,pmdec)
        barycenter <- rbind(obs1[,c('ra','dec','parallax','pmra','pmdec')],obs[c('ra','dec','parallax','pmra','pmdec')],obs[c('ra','dec','parallax','pmra','pmdec')])
    }
    companion <- cbind(ra=planet$ra,dec=planet$dec,pmra=planet$pmra,pmdec=planet$pmdec)
    return(list(out=astro,rv.pm=rv.pm,planet=companion,barycenter=barycenter,rel=planet$rel))
}
...
astrometry.kepler <- function(pars.kep,tt=NULL,bases=rep('natural',10),Ein=NULL,pa=TRUE){
    if(out$Nastro>0){
        if(is.null(tt)){
            if(!grepl('hgca',astro.type)){
                tt <- out$astrometry[,1]%%24e5-tmin%%24e5
            }else{
                tt <- c(as.numeric(out$astrometry[1,c('epoch_ra','epoch_dec')]),as.numeric(out$astrometry[2,c('epoch_ra','epoch_dec')]))%%24e5-tmin%%24e5
            }
        }
...
###add relativistic effects
            ...
            if(out$relativity){
                ...
                if(binarity) EC <- tmp$EC
                rvgP <- tmp$rvgT
                rvsP <- tmp$rvsT
                drvP <- drvP+rvgP+rvsP+tmp$drvT#account for relativistic and astroemtric effect
#cat('GR+SR:',head(tmp$rvgs),'\n')
                if(binarity) drvC <- drvC+tmp$rvgsC+tmp$drvC
            }else{
                for(h in 1:Np.kep){
                    m <- (pp$Mo[h]+2*pi*tt/pp$P[h])%%(2*pi)#mean anomaly
                    if(prior.kep!='e0'){
...
###
            ...
            if(binarity){
                    T <- 2*atan(sqrt((1+pp$e[1])/(1-pp$e[1]))*tan(EC/2))
                    rv <- pp$K[1]*(cos(pp$omega[1]+T)+pp$e[1]*cos(pp$omega[1]))
                    drvC <- drvC-rv*Mstar/Mps[1]
            }
        }
...
###assign parameters and data
            ...
            if(!sim.kep){
                dVr.kep[[out$ins[j2]]] <- drvP[out[[out$ins[j2]]]$index]
                tt <- out[[out$ins[j2]]]$RV[,1]-tmin
            }else{
                dVr.kep <- drvP
            }
            if(!kep.only){
                ...
                trend <- cal.trend(a=at,b=b,t=tau[out[[out$ins[j2]]]$index]/time.unit)#
                                        #        cat('range(trend)=',range(trend),'\n')
                dVr.kep[[out$ins[j2]]] <- dVr.kep[[out$ins[j2]]]+trend

####PM-induced trend
                if(any(names(rv.pm)==ins[j2])){
                    dVr.kep[[out$ins[j2]]] <- dVr.kep[[out$ins[j2]]]+rv.pm[[out$ins[j2]]]
                }
                if(nqp[1]>0){
...
#####binary companion RV
    if(binarity & offset & !sim.kep){
        ...
        for(j in 1:Nins){
            ...
            drvC[out$ind.binary[[ins]]] <- drvC[out$ind.binary[[ins]]]+pars.kep[paste0('b_',ins)]
        }
    }
#    cat('drvC=',head(drvC),'\n')
    return(list(rv=dVr.kep,rvg=rvgP,rvs=rvsP,rvc=rvc,rvm=rvm,tau=tau,drvC=drvC))
}
...
loglikelihood <- function(pars,bases=rep('natural',10),RVonly=FALSE,nqp=NULL,prediction=FALSE,indrel=NULL,verbose=FALSE){
    ...
    if((out$Nastro>0 | any(names(out)=='rel')) & !RVonly){
...
#    cat('RV logLike=',logLike,'\n')
    ...
    if(out$Nastro>0){
        ...
        }else if(grepl('--',astro.type)){
            res <- out$astrometry[,c('pmra','pmdec')]-rbind(c(PM.kep[1,4],PM.kep[2,5]),c(PM.kep[3,4],PM.kep[4,5]))
            for(j in 1:Nastro){
                ll.astro <- ll.astro+dmvnorm(res[j,], mean=rep(0,2), sigma=cov.astro[4:5,4:5,j], log=TRUE)
                if(prediction) res.all[[paste0('astro',j)]] <- res[j,]
            }
        }else{
            ress <- rbind(rep(0,4),rep(0,4))
            for(j in 1:nepoch){
                s <- 0
                if(j<=2 & any(names(pars)=='logJ.hip')) s <- exp(pars['logJ.hip'])
                if(j>=3 & any(names(pars)=='logJ.gaia')) s <- exp(pars['logJ.gaia'])
                if(j==1 | j==3){
                    ind <- ceiling(j/2)
                    astro <- unlist(out$astrometry[ind,c('ra','pmra')])#data
                    res <- pred <- PM.kep[ind,c(1,4)]#model
                    res[1] <- (astro['ra']-pred[1])*cos(PM.kep[2]/180*pi)*3.6e6#deg to mas; dalpha to dalpha*
                    res[2] <- astro['pmra']-pred[2]
                    ll <- dmvnorm(res, mean=rep(0,2), sigma=cov.astro[c(1,4),c(1,4),ind]*(1+s), log=TRUE)
                    ress[ind,c(1,3)] <- res
                }else{
                    ind <- ceiling(j/2)
                    astro <- unlist(out$astrometry[ind,c('dec','pmdec')])
                    res <- pred <- PM.kep[ind,c(2,5)]
                    res[1] <- (astro['dec']-pred[1])*3.6e6#deg to mas; dalpha to dalpha*
                    res[2] <- astro['pmdec']-pred[2]
                    ll <- dmvnorm(res, mean=rep(0,2), sigma=cov.astro[c(2,5),c(2,5),ind]*(1+s), log=TRUE)
                    ress[ind,c(2,4)] <- res
                }
                if(prediction){
                    res.all[[paste0('astro1')]] <- ress[1,]
                    res.all[[paste0('astro2')]] <- ress[2,]
                }
                ll.astro <- ll.astro+ll
            }
        }
        if(verbose) cat('ll for absolute astrometry=',ll.astro,'\n')
    ...
    if(any(names(out)=='rel')){
        for(j in 1:Np){
            if(length(indrel[[j]])>0){
                ...
                for(i in inss){
                    ll <- 0
#                    ind0 <- which(out$rel[[n]][[i]][,'cov.ra.dec']==0)
#                    ind1 <- which(out$rel[[n]][[i]][,'cov.ra.dec']!=0)
                    ...
                    if(rel.type=='rd'){
                        ...
                        if(any(colnames(out$rel[[n]][[i]])=='dpmra')){
                            res <- cbind(res,out$rel[[n]][[i]][,'dpmra']-rel$pmraB[[n]][[i]],out$rel[[n]][[i]][,'dpmdec']-rel$pmdecB[[n]][[i]])
...
#                        ll <- ll+sum(dnorm(res[ind0,], mean=0, sd=erel[ind0,], log=TRUE))
                        ...
                        for(k in 1:nrow(out$rel[[n]][[i]])){
                            ll <- ll+dmvnorm(res[k,], mean=rep(0,length(res[k,])), sigma=cov.rel[[k]]*(1+s), log=TRUE)
                        }
                    }
...
####binary
   if(length(out$data.binary)>0){
       ...
       for(j in 1:Nins){
           ins <- out$ins.binary[j]
           res <- out$data.binary[out$ind.binary[[ins]],2]-rvp$drvC[out$ind.binary[[ins]]]
